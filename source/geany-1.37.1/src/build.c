...
static guint build_groups_count[GEANY_GBG_COUNT] = { 6, 4, 6 };
static guint build_items_count = 16;
...
static gchar *build_replace_placeholder(const GeanyDocument *doc, const gchar *src)
{
	...
	if (doc != NULL && doc->file_name != NULL)
	{
		...
		/* replace %o with the project options */
		replacement = project_get_options();
		utils_string_replace_all(stack, "%o", replacement);
                /* replace cr or lf with space */
	        utils_string_replace_all(stack, "\r\n", "\n");
	        utils_string_replace_all(stack, "\n", " ");
		g_free(replacement);

                /* replace %b with the project baudrate */
                replacement = NULL;
		replacement = project_get_baudrate();
                if ((replacement == NULL || strlen(replacement)) == 0) {
                   replacement = g_strdup("115200");
                }
		utils_string_replace_all(stack, "%b", replacement);
		g_free(replacement);

                /* replace %n with the project name */
                replacement = NULL;
		replacement = project_get_project_name();
                if ((replacement != NULL && strlen(replacement)) != 0) {
		               utils_string_replace_all(stack, "%n", replacement);
                }
		g_free(replacement);

                /* replace %x with the project loader */
                replacement = NULL;
		replacement = project_get_loader();
                if ((replacement == NULL || strlen(replacement)) == 0) {
                   replacement = g_strdup("");
                }
		utils_string_replace_all(stack, "%x", replacement);
		g_free(replacement);
        }
								...
								GeanyFiletype *ft)
{
  ...
  clear = gtk_button_new_with_label("reset");
	...
	label = gtk_label_new(NULL);
	ui_label_set_markup(GTK_LABEL(label), "<i>%s</i>",
		_("%b, %d, %e, %f, %p, %l, %n, %o, %x are substituted in command and directory fields, see manual for details."));
	gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
	gtk_table_attach(table, label, 0, DC_N_COL, row, row + 1, GTK_FILL, GTK_FILL | GTK_EXPAND,
		entry_x_padding, entry_y_padding);

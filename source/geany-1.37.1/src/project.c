...
static void destroy_project(gboolean open_default)
{
	...
	g_free(app->project->options);
	g_free(app->project->baudrate);
	g_free(app->project->loader);
...
static void create_properties_dialog(PropertyDialogElements *e)
{
	...
	e->options = ui_lookup_widget(e->dialog, "textview_project_dialog_options");
	e->baudrate = ui_lookup_widget(e->dialog, "entry_project_dialog_baudrate");
	e->loader = ui_lookup_widget(e->dialog, "entry_project_dialog_loader");
	...
	ui_entry_add_clear_icon(GTK_ENTRY(e->baudrate));
	ui_entry_add_clear_icon(GTK_ENTRY(e->loader));
...
static void show_project_properties(gboolean show_build)
{
	...
	gtk_entry_set_text(GTK_ENTRY(e.baudrate), p->baudrate);
	gtk_entry_set_text(GTK_ENTRY(e.loader), p->loader);
	...
	/* set text */
	buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(e.options));
	gtk_text_buffer_set_text(buffer, p->options ? p->options : "", -1);

	/* set text */
	entry_text = g_strdup(gtk_entry_get_text(GTK_ENTRY(e.baudrate)));
	gtk_entry_set_text(GTK_ENTRY(e.baudrate), entry_text);
	g_free(entry_text);

	/* set text */
	entry_text = g_strdup(gtk_entry_get_text(GTK_ENTRY(e.loader)));
	gtk_entry_set_text(GTK_ENTRY(e.loader), entry_text);
	g_free(entry_text);
...
static gboolean update_config(const PropertyDialogElements *e, gboolean new_project)
{
	...
	if (! new_project)	/* save properties specific fields */
	{
		...
		/* get and set the project options */
		buffer = gtk_text_view_get_buffer(GTK_TEXT_VIEW(e->options));
		gtk_text_buffer_get_start_iter(buffer, &start);
		gtk_text_buffer_get_end_iter(buffer, &end);
		SETPTR(p->options, gtk_text_buffer_get_text(buffer, &start, &end, FALSE));

		/* get and set the project baudrate */
		if (strlen(gtk_entry_get_text(GTK_ENTRY(e->baudrate))) > 0) {
			p->baudrate = g_strdup(gtk_entry_get_text(GTK_ENTRY(e->baudrate)));
			}
		else {
			p->baudrate = g_strdup("115200");
		}

		/* get and set the project loader */
		if (strlen(gtk_entry_get_text(GTK_ENTRY(e->loader))) > 0) {
			p->loader = g_strdup(gtk_entry_get_text(GTK_ENTRY(e->loader)));
		}
		else {
			p->loader = g_strdup("");
		}
...
static gboolean load_config(const gchar *filename)
{
	...
	p->options = utils_get_setting_string(config, "project", "options", "");
	p->baudrate = utils_get_setting_string(config, "project", "baudrate", "");
	if ((p->baudrate == NULL) || (strlen(p->baudrate) == 0)) {
		p->baudrate = g_strdup("115200");
	}
	p->loader = utils_get_setting_string(config, "project", "loader", "");
	if ((p->loader == NULL) || (strlen(p->loader) == 0)) {
		p->loader = g_strdup("");
	}
...
static gboolean write_config(void)
{
	...
	if (p->options)
		g_key_file_set_string(config, "project", "options", p->options);
	if (p->baudrate)
		g_key_file_set_string(config, "project", "baudrate", p->baudrate);
	if (p->loader)
		g_key_file_set_string(config, "project", "loader", p->loader);
	if (p->file_patterns)
...
/* Constructs the project's project name which can be used for "Make".
 * The result is an absolute string in UTF-8 encoding.
...
/* Constructs the project's options for compile, build and execute commands.
 * If there is no project or project's options are invalid, NULL will be returned.
 * The returned string should be freed when no longer needed. */
gchar *project_get_options(void)
{
	GeanyProject *project = app->project;

	if (project && !EMPTY(project->options))
	{
		return g_strdup(project->options);
	}
	return NULL;
}


/* Constructs the project's baudrate for payload commands.
 * If there is no project or project's baudrate are invalid, NULL will be returned.
 * The returned string should be freed when no longer needed. */
gchar *project_get_baudrate(void)
{
	GeanyProject *project = app->project;

	if (project && !EMPTY(project->baudrate))
	{
		return g_strdup(project->baudrate);
	}
	return NULL;
}


/* Constructs the project's loader for payload commands.
 * If there is no project or project's loader are invalid, NULL will be returned.
 * The returned string should be freed when no longer needed. */
gchar *project_get_loader(void)
{
	GeanyProject *project = app->project;

	if (project && !EMPTY(project->loader))
	{
		return g_strdup(project->loader);
	}
	return NULL;
}

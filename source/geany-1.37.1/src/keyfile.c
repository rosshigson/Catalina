...
static gchar *get_session_file_string(GeanyDocument *doc)
{
	...
	if ((basedir != NULL) && (strncmp(doc->file_name, basedir, baselen) == 0)) {
		if (strncmp(&doc->file_name[baselen], G_DIR_SEPARATOR_S, strlen(G_DIR_SEPARATOR_S)) == 0) {
			/* don't include separator */
			offset = baselen + strlen(G_DIR_SEPARATOR_S);
		}
		else {
			offset = baselen;
		}
		locale_filename = utils_get_locale_from_utf8(&doc->file_name[offset]);
	}
	else {
...
static gboolean open_session_file(gchar **tmp, guint len)
{
	...
	{
		//try opening file in basedir
		gchar *short_name;
		gchar *locale_long_name;
		gchar *unescaped_long_name;
		short_name = g_uri_unescape_string(tmp[7], NULL);
		unescaped_long_name = g_build_filename(basedir, short_name, NULL);
		locale_long_name = utils_get_locale_from_utf8(unescaped_long_name);
		{
			GeanyFiletype *ft = filetypes_lookup_by_name(ft_name);
			GeanyDocument *doc = document_open_file_full(
				NULL, locale_long_name, pos, ro, ft, encoding);
		...
		g_free(locale_long_name);
		g_free(unescaped_long_name);
	  g_free(short_name);
	}
  else
	{

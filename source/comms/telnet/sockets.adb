...
--  This package has been modified by RJH to allow transmission and reception
--  of TCP Urgent (OOB) data. The changes are marked in the package, and the
--  package was renamed (from GNAT.Sockets to Sockets) to allow it to coexist 
--  with the existing GNAT package.
...
with Sockets.Thin;               use Sockets.Thin; -- modified by RJH
with Sockets.Thin_Common; use Sockets.Thin_Common; -- modified by RJH

with Sockets.Linker_Options;                       -- modified by RJH
pragma Warnings (Off, Sockets.Linker_Options);     -- modified by RJH
--  Need to include pragma Linker_Options which is platform dependent

with Sockets.Poll;        -- package name modified by RJH
...
package body Sockets is        -- package name modified by RJH

   --  Control Flags added by RJH
   SIOCATMARK           : constant :=       1074033415; -- Added by RJH
   --  Socket Options added by RJH
   SO_OOBINLINE         : constant :=              256; -- Added by RJH
   -- Send Flags added by RJH
   MSG_OOB              : constant :=                1; -- Added by RJH
   MSG_PEEK             : constant :=                2; -- Added by RJH
   MSG_DONTROUTE        : constant :=                4; -- Added by RJH

--   package Task_Lock renames GNAT.Task_Lock; -- Added by RJH
   ...
   Requests : constant array (Request_Name) of SOSC.IOCTL_Req_T :=
                (Non_Blocking_IO => SOSC.FIONBIO,
                 ...
                 Urgent_Data     => SIOCATMARK);  -- added by RJH
   ...
   Options : constant array (Specific_Option_Name) of C.int :=
               (Keep_Alive          => SOSC.SO_KEEPALIVE,
                ...
                Urgent_Inline       => SO_OOBINLINE); -- added by RJH
   ...
   begin
      ...
      case Request.Name is
         ...
         when Urgent_Data =>                         -- added by RJH
            Request.No_Urgent_Data := not (Arg = 0); -- added by RJH
   ...
   function Resolve_Error
     (Error_Value : Integer;
      From_Errno  : Boolean := True) return Error_Type
   is
      use Sockets.SOSC;          -- modified by RJH
...
end Sockets;        -- package name modified by RJH

 ...
 *
 * Version 4.9.5 - Fix bug in assembling the output of optimization phase 4.
 *                 Now, Phase 14 is always called beforehand to insert the
 *                 necessary "align" statements. Otherwise, the offsets
 *                 calculated by phase 5 may be incorrect.
 *
 * Version 4.9.6 - Fix bug in optimization phase 12. It now correctly skips 
 *                 alignl statements.
 *
...
int do_optimize(char *fullname) {
   ...
   code = do_phase_14(OPTIMIZER_PHASE_4_OUTPUT_NAME);
   if (code != 0) {
      return code;
   }
   remove_unquoted(OPTIMIZER_PHASE_4_OUTPUT_NAME);
   code = rename_unquoted(OPTIMIZER_PHASE_14_OUTPUT_NAME, OPTIMIZER_PHASE_4_OUTPUT_NAME);

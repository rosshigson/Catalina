/*
 * Optimize - PASM level CMM optimizer for the Catalina compilation system.
 *
 * Version 3.7 - first version for CMM
 *
...
#define ASSEMBLE_OUTOPT    " -o" /* note - space before, but no space after */
...
#define ASSEMBLE_HS        "homespun -i1 -b -L . -D CATALINA_OPTIMIZE "
#define LISTOPT_HS         " -d"  /* note - space before, but no space after */
#define LIBOPT_HS          " -L " /* note - space before and after */
#define VERBOSE_HS         "-i3 " /* note - space before and after */
#define QUIET_HS           ""
...
#define ASSEMBLE_P2        "p2_asm -D CATALINA_OPTIMIZE "
#define LISTOPT_P2         ""  
#define LIBOPT_P2          " -I " /* note - space before and after */
#define VERBOSE_P2         "" 
#define QUIET_P2           ""

#define OPTIMIZE_FILENAME  "Catalina.spin"

#define OPTIMIZE_FILENAME_P2  "Catalina.spin2"

#define CMD_DEFINE_STRING  "-D " /* note - space after */

#define PHASE_0_COMMAND    "cmmopt_0 "
#define PHASE_1_COMMAND    "cmmopt_1 "
#define PHASE_2_COMMAND    "cmmopt_2 "
#define PHASE_3_COMMAND    "cmmopt_3 "
#define PHASE_4_COMMAND    "cmmopt_4 "
#define PHASE_5_COMMAND    "cmmopt_5 "
#define PHASE_6_COMMAND    "cmmopt_6 "
#define PHASE_7_COMMAND    "cmmopt_7 "
#define PHASE_8_COMMAND    "cmmopt_8 "
#define PHASE_9_COMMAND    "cmmopt_9 "
#define PHASE_10_COMMAND   "cmmopt_10 "
#define PHASE_11_COMMAND   "cmmopt_11 "
#define PHASE_12_COMMAND   "cmmopt_12 "
#define PHASE_13_COMMAND   "cmmopt_13 "
#define PHASE_14_COMMAND   "cmmopt_14 "
...
static char * input_file[MAX_FILES + 3];     /* should only need one */
...
static char assemble_command[MAX_LINELEN + 1] = ASSEMBLE_HS;
static char assemble_verbose[MAX_LINELEN + 1] = VERBOSE_HS;
static char assemble_quiet[MAX_LINELEN + 1]   = QUIET_HS;
static char assemble_options[MAX_LINELEN + 1] = "";

static int homespun = 1;   /* use Homespun by default */
...
void help(char *my_name) {
   ...
   fprintf(stderr, "          -d        generate listing (homespun syntax)\n");
   fprintf(stderr, "          -D        define symbol\n");
   fprintf(stderr, "          -e        eeprom output\n");
   fprintf(stderr, "          -ilevel   set information level (note no space!)\n");
   fprintf(stderr, "          -I path   path to libraries (spinnaker syntax)\n");
   fprintf(stderr, "          -k        suppress banner\n");
   fprintf(stderr, "          -l        generate listing (spinnaker syntax)\n");
   fprintf(stderr, "          -L path   path to libraries (homespun syntax)\n");
      ...
      if (i > 0) {
         if (argv[i][0] == '-') {
            ...
            switch (argv[i][1]) {
               ...
               case 'I':
                  if (prop_vers < 2) {
                     // this option implies spinnaker (P1 only)
                     spinnaker = 1;
                     p2asm = 0;
                  }
                  if (strlen(argv[i]) == 2) {
                  ...
                  break;
               case 'L':
                  // this option implies homespun (P1 only)
                  if (prop_vers < 2) {
                     spinnaker = 0;
                     p2asm = 0;
                  }
               ...
               case 'M':
                  if (homespun == 0) {
                     if (verbose) {
                       fprintf(stderr, "note: cannot use -M with -b\n");
                     }
                    fprintf(stderr, "option -M ignored\n");
                  }
                  else {
                     memory = 1;
                     if (strlen(argv[i]) == 2) {
...
int do_assemble(char *fullname, char *outname, int listing, char *library) {
   ...
   if (listing) {
      if (spinnaker) {
         ...
         safecat(assemble, LISTOPT_HS, MAX_LINELEN);
      }
   }
   if (format) {
   ...
   if (library != NULL) {
      if (prop_vers == 2) {
         ...
         safecat(assemble, LIBOPT_HS, MAX_LINELEN);
      }
      safecat(assemble, "\"", MAX_LINELEN);
...
int do_phase_14(char *fullname) {
   ...
   safecpy(optimize, PHASE_14_COMMAND, MAX_LINELEN);
   safecat(optimize, fullname, MAX_LINELEN);
   safecat(optimize, " >", MAX_LINELEN);
   safecat(optimize, OPTIMIZER_PHASE_14_OUTPUT_NAME, MAX_LINELEN);
   if (verbose) {
      printf("command = %s\n", optimize);
   }
#if DO_EXECUTE
...
int do_optimize() {
   char fullname[MAX_LINELEN + 1] = "";
   int   code;

#if COMPILE_IN_PLACE
   if (prop_vers == 2) {
      strncpy(fullname, OPTIMIZE_FILENAME_P2, MAX_LINELEN);
   }
   else {
      strncpy(fullname, OPTIMIZE_FILENAME, MAX_LINELEN);
   } 
#else
   strncpy(fullname, target_path, MAX_LINELEN);
   strncat(fullname, PATH_SEP, MAX_LINELEN);
   if (prop_vers == 2) {
      strncpy(fullname, OPTIMIZE_FILENAME_P2, MAX_LINELEN);
   }
   else {
   ...
   if (olevel > 1) {
      ...
      if (olevel >= 4) {
         ...
         code = do_phase_14(OPTIMIZER_PHASE_13_OUTPUT_NAME);
         if (code != 0) {
            return code;
         }
      }
      else {
         code = do_phase_14(OPTIMIZER_PHASE_6_OUTPUT_NAME);
         if (code != 0) {
            return code;
         }
      }
      code = rename_unquoted(OPTIMIZER_PHASE_14_OUTPUT_NAME, fullname);
      ...
      code = do_phase_14(OPTIMIZER_PHASE_10_OUTPUT_NAME);
      if (code != 0) {
         return code;
      }
      remove_unquoted(fullname);
   ...
   if (cleanup) {
      ...
      remove_unquoted(OPTIMIZER_PHASE_13_OUTPUT_NAME);
   }
   return 0;
}
...
int main (int argc, char *argv[]) {
   ...
   // check for files that should be optimized (a bit of a hack)
   ...
   if (name != NULL) {
      ...
      if (strcmp(name, OPTIMIZE_FILENAME) == 0) {
         optimize = 1;
      }
      if (strcmp(name, OPTIMIZE_FILENAME_P2) == 0) {
         optimize = 1;
      }
      if (strncmp(name, CMM_PREFIX, 4) == 0) {
         optimize = 1;
      }
   }

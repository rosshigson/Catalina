...
MAIN()
{
  ...
  while (awka_getline(a_TEMP, awka_dol0(0), awka_gets(a_bivar[a_FILENAME]), FALSE, TRUE)->dval > 0 && awka_setNF())
  {
    ...
    {
      if (awka_vartrue(known_function_fn(awka_arg1(a_TEMP, awka_doln(1, 0)))))
      {
    ...
    if ((!strcmp(awka_gets(awka_doln(1, 0)), "long")) && !strcmp(awka_gets(awka_doln(2, 0)), "I32_SPILL"))
    {
      if (strcmp(f_awk->ptr, "no function"))
      {
...
main(int argc, char *argv[])
{
  ...
  awka_varinit(_lits0_awka); awka_strcpy(_lits0_awka, "no function");
  awka_varinit(_lits1_awka); awka_strcpy(_lits1_awka, "}");
  awka_varinit(_lits2_awka); awka_strcpy(_lits2_awka, "long");
  awka_varinit(_lits3_awka); awka_strcpy(_lits3_awka, "I32_SPILL");

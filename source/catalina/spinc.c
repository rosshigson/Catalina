...
#define DEFAULT_STACK_SIZE 80 // 80 bytes (20 longs)
...
static void print_p2_blob_defines(char* p, char* name, int *datstart, int *datlen)
{
   ...
   *datstart = cp2->catalina_code;
   ...
   if (((blob_layout != LMM_LAYOUT) && (blob_layout != CMM_LAYOUT) && (blob_layout != NMM_LAYOUT))
   ||  (cp2->catalina_ends < cp2->catalina_code)) {
     fprintf(stderr, "\nError: Binary file is not a Catalina CMM, LMM or NMM program\n");
     exit(1);
   }

...
/* Extracted from Gawk-3.0.50, August 2000 by Andrew Sumner for inclusion
   with Awka.  Minor modifications have been made to allow Awka's data
   structures to call these functions. */
...
#if defined (__GNUC__) || (STDC_HEADERS) 
#include <stdlib.h>
#else
#include <sys/types.h>
...
#define REALLOC(p, t, n) ((p) = (t *) xrealloc((void *) (p), (n) * sizeof (t)))
...
lex()
{
  ...
  abort();
  return DFAEND;        /* keeps pedantic compilers happy. */
}
...
/* Search through a buffer looking for a match to the given struct dfa.
   ...
   character after the match's end, or NULL if none is found.  Begin points to
     ...
     int searchflag;
{
  char *string = NULL;
  struct dfa *d;
  MALLOC(d, struct dfa, 1);
  memset(d, 0, sizeof(struct dfa));
  
  string = malloc(len+1);
  strcpy(string, s);
  if (len > 1 && *string == '/' && string[len-1] == '/')
  {
     ...
     struct dfa *d;
{
  ...
  for (i = 0; i < d->tralloc; ++i)
    if (d->trans[i])
      ...
      free((void *) d->fails[i]);
  if (d->realtrans) free((void *) d->realtrans);
  if (d->fails) free((void *) d->fails);
  if (d->newlines) free((void *) d->newlines);
  if (d->success) free((void *) d->success);

...
#define BLACKCAT_COMPATIBLE 1 // 1 for full blackcat compatibility
...
static int    mem_model = -1; // On P1 0 = TINY LMM, 2 = SMALL XMM, 5 = LARGE XMM, 8 = CMM
                              // On P2 100 = TINY LMM, 108 = CMM, 111 = NMM
         ...
         while (p != NULL) {
            if (p->type == linenum_t) {
               ...
               if ((c != NULL) && (c->d.locn->value == l->d.locn->value)) {
                  if (diagnose) {
                     printf("delete file %d, line %d - same addr as line %d\n",
                        l->d.src, l->d.num, c->d.num);
                  }
                  free(l);
               }
               else {
            ...
            case array_t:
               ...
               if (isdigit(*str)) {
                  ...
                  str = parse_var(file, line, str, 
                           &index_base, &index_leaf, &index_offs);
                  if (*str != ']') {
...
void print_char_1(struct location *locn, int offs, int relocate) {
   unsigned int value;
   char value_1;
   unsigned int addr;
   unsigned int fp;
...
void print_scalar(
   ...
   if (leaf->t.d_type == func_t) {
     ...
     if (strncmp(leaf->t.type, "*", 1) == 0) {
        ...
        print_char_1 (locn, offs, relocate);
     }
...
struct node *print_var(int file, int line, char *str, int relocate) {
   ...
   end = parse_var(file, line, str, &base, &leaf, &offs);
   if ((*end != '\0') && (*end != '\n')) {
      printf("unexpected characters after variable\n");
   }

   if (base != NULL) {
      if (diagnose) {
         printf("variable = %s%s \n", base->v.name, 
            (addr ? " (address)": (deref ? " (dereferenced)" : "")));
         printf("address = ");
...
char *get_value(char *str, int * new_value, int *value_len) {
   char *start;
   int i;
   int float_flag = 0;
   ...
   *value_len = 4;
   ...
   if ((i = is_command(str, "byte", 1))) {
      str += i;
      *value_len = 1;
   }
   if ((i = is_command(str, "char", 1))) {
      str += i;
      *value_len = 1;
   }
   else if ((i = is_command(str, "word", 1))) {
      str += i;
      *value_len = 2;
   }
   else if ((i = is_command(str, "short", 1))) {
      str += i;
      *value_len = 2;
   }
   else if ((i = is_command(str, "long", 1))) {
      str += i;
      *value_len = 4;
   }
   else if ((i = is_command(str, "float", 1))) {
      str += i;
      *value_len = 4;
      float_flag = 1; // decode as float
   }
   else if ((i = is_command(str, "double", 1))) {
      str += i;
      *value_len = 4;
      float_flag = 1; // decode as float
   }
...
void update_char_1(struct location *locn, int offs, int val, int len, int relocate) {
   unsigned int value;
   char value_1 = val;
   unsigned int addr;
   unsigned int fp;
...
void update_integer_2(struct location *locn, int offs, int val, int len, int relocate) {
   ...
   switch (locn->type) {
      case mem :
         ...
         if (mem_model == 5) {
            if (blackbox_xmm_read(mem_model, addr, (unsigned int *)&value) == 0) {
               value &= ~(0xffff << ((addr & 2)<<4));
               val = (val & 0xffff) << ((addr & 2)<<4);
               value |= val;
               value |= val;
               if (blackbox_xmm_write(mem_model, addr, value, 4) == 0) {
                  if (verbose) {
...
void update_scalar(
   ...
   if (leaf->t.d_type == func_t) {
      ...
      if (strncmp(leaf->t.type, "*", 1) == 0) {
         ...
         update_char_1 (locn, offs, new_value, value_len, relocate);
      }
      else if (strcmp(leaf->t.type, "signed char") == 0) {
         if (verbose) {
            printf("signed char @ ");
...
struct node *update_expr(int file, int line, char *str, int relocate) {
   ...
   if (base != NULL) {
      ...
      tok = get_value(end, &new_value, &value_len);
      if (tok == end) {
         printf("invalid value specified\n");
         return NULL;
      }
      if ((*tok != '\0') && (*tok != '\n')) {
         printf("unexpected characters after value\n");
         return NULL;
      }
      if (diagnose) {
...
void file_cmd(char *args, int len) {
   int src = 0;
   int i = 0;
   char *tok = NULL;
   char *delim = " \n\t,:-=";
...
void list_cmd(char *args, int len) {
   int i = 0;
...
char * lookup_register(char *str, int *reg) {
   ...
   if (strcmp_i(str,"PC") == 0) {
      *reg = PC_OFF;
      return str+2;
   }
   else if (strcmp_i(str, "SP") == 0) {
      *reg = SP_OFF;
      return str+2;
   }
   else if (strcmp_i(str, "FP") == 0) {
      *reg = FP_OFF;
      return str+2;
   }
   else if (strcmp_i(str, "RI") == 0) {
      *reg = RI_OFF;
      return str+2;
   }
   else if (strcmp_i(str, "BC") == 0) {
      *reg = BC_OFF;
      return str+2;
   }
   else if (strcmp_i(str, "BA") == 0) {
      *reg = BA_OFF;
      return str+2;
   }
   else if (strcmp_i(str, "BZ") == 0) {
      *reg = BZ_OFF;
      return str+2;
   }
   else if (strcmp_i(str, "CS") == 0) {
      *reg = CS_OFF;
      return str+2;
   }
...
void read_cmd(char *args, int len) {
   int i = 0;
   char *tok = NULL;
   char *end = NULL;
   char *delim = " \n\t,:-=";
   ...
   end = &args[len] + strlen(&args[len]);
   tok = strtok(&args[len], delim);
   if (tok != NULL) {
      tok = lookup_register(tok, &location);
      if (location < 0) {
         if ((i = is_command(tok, "cog", 1))) {
            tok = strtok(NULL, delim);
            if (tok == NULL) {
            ...
            mem_type = 0;
            if (!get_address(&tok, &location)) {
               printf("invalid address specified\n");
               return;
            }
         }
         else if ((i = is_command(tok, "hub", 1))) {
            tok = strtok(NULL, delim);
            if (tok == NULL) {
            ...
            if (!get_address(&tok, &location)) {
               ...
               if ((*tok == '*') || (*tok == '&')) {
                  printf("dereferencing or address operators not allowed here\n");
                  return;
               }
         ...
         else if ((i = is_command(tok, "xmm", 1))) {
            tok = strtok(NULL, delim);
            if (tok == NULL) {
   ...
   if ((tok == NULL) || (*tok == '\0') || (*tok == '\n')) {
      ...
      end = get_value(tok, &count, &count_len);
      while (isspace(*tok)) {
         tok++;
      }
      if ((*end != '\0') && (*end != '\n')) {
...
void write_cmd(char *args, int len) {
   int i = 0;
   char *tok = NULL;
   char *end = NULL;
   char *delim = " \n\t,:-=";
   ...
   if (tok != NULL) {
      ...
      if ((*tok == '\n') || (*tok == '\0')) {
         printf("no new value specified\n");
         return;
      }
      if (*tok == '=') {
         tok++;
      }
      end = get_value(tok, &new_value, &value_len);
      if (tok == end) {
         printf("invalid value specified\n");
         return;
      }
...
void print_cmd(char *args, int len) {

   if (debugger_line != NULL) {
      print_var(debugger_line->d.src, debugger_line->d.num, &args[len], 1);
   }
   else {
...
void update_cmd(char *args, int len) {

   if (debugger_line != NULL) {
      update_expr(debugger_line->d.src, debugger_line->d.num, &args[len], 1);
   }
   else {
      printf("error - no current context\n");
   }
...
//            then continue
void continue_cmd(char *args, int len) {
   unsigned int addr;

   while (isspace(args[len])) {
      len++;
   }
...
void next_cmd(char *args, int len) {
   unsigned int addr;
...
void into_cmd(char *args, int len) {
   unsigned int addr;
...
void out_cmd(char *args, int len) {
   unsigned int addr;
...
void external_cmd(char *args, int len) {
   unsigned int addr;
...
void trace_cmd(char *args, int len) {
   unsigned int addr;
   unsigned int fp;
   unsigned int ret;
   struct node *d;
...
void breakpoint_cmd(char *args, int len) {
   int src;
...
void delete_cmd(char *args, int len) {
   int i = 0;
   int f = current_file - 1;
   int l = current_line;
   char *tok = NULL;
...
void mode_cmd(char *args, int len) {
   char *tok = NULL;
   char *delim = " \n\t,:-=";
   int  i;
   int  new_port = 0;
...
void xmm_cmd(char *args, int len) {
   char *tok = NULL;
   char *delim = " \n\t,:-=";
   int  actual_model = -1;
...
void step_cmd(char *args, int len) {
   int len2;
   char *tok = NULL;
   char *delim = " \n\t,:-=";
...
void verbose_cmd(char *args, int len) {
   int i;
   char *tok = NULL;
   char *delim = " \n\t,:-=";
...
void help_cmd(char *args, int len) {
      ...
      if (exit_confirmed) {
         ...
         if (at_initial_brkpt) {
            if (blackbox_go(mem_model, NO_OPERATION) != 0) {
               printf("cannot resume program execution\n");
            }
         }
         else {
...
void help(char *my_name) {
  ...
  fprintf(stderr, "          -L name   execure the named Lua script after openint the port\n");

...
[
    ...
    s.match /([0-9a-f]+) +(\d+) +(.+)/
    if symbols.has_key? $3
        symbols[$3][:value2] = $1.to_i 16
    else
        symbols[$3] = {value: $1.to_i(16), type: $2}
    end
end


File.open "p2asmsym2.h","w" do |f|
    ...
    symbols.each_pair do |name,v|
        f.write("    {0x%x, 0x%x, %d, %s},\n" % [v[:value],v[:value2]||0,v[:type],name.inspect])
    end
    f.write "};\n"
end

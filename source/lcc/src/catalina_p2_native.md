...
reg:  ADDF4(reg,reg) "# calld PA,#FADD ' ADDF4\n" 5
...
#define preg1(f) ((f)[getregnum(p->x.kids[1])]->x.name)
    ...
    */
      ...
      print(" calld PA,#FADD ' ADDF4\n");
   }
   else if (op == SUB+F) {
      setup_r0_and_r1(preg0(intreg), preg1(intreg)); 
      print(" calld PA,#FSUB ' SUBF4\n");
   }
   else if (op == MUL+F) {
      setup_r0_and_r1(preg0(intreg), preg1(intreg)); 
      print(" calld PA,#FMUL ' MULF4\n");
   }
   else if (op == DIV+F) {
      setup_r0_and_r1(preg0(intreg), preg1(intreg)); 
...
#endif
      if (((saveimask | savefmask) & 0xfffffffe) != 0) {
         print (" calld PA,#POPM ' restore registers\n");
      }
   }
   if (ismain) {

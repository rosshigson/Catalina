...
Tree simplify(int op, Type ty, Tree l, Tree r) {
	...
	switch (op) {
		...
		case CVU+I:
			...
			break;
		case CVP+U:
			xcvtcnstp(P,(intptr_t)l->u.v.p,ty,u,l->u.v.p);
			break;
		case CVU+P:
			xcvtcnstp(U,(intptr_t)l->u.v.u,ty,p,l->u.v.u);
			break;
		case CVP+P:
			xcvtcnst(P,l->u.v.p,ty,p,l->u.v.p);
			break;
		case CVI+U:
			xcvtcnst(I,(unsigned)l->u.v.i,ty,u,((unsigned long)l->u.v.i)&ones(8*ty->size));
			break;
		case CVU+U:

...
/* last_src and last_dst were added by rjh to enable trivial "peephole" optimization */
int last_src = -1;
int last_dst = -1;
...
static char *outp(intptr_t n, int base, FILE *f, char *bp) {
  char buf[25], *s = buf + sizeof buf;
...
void print(const char *fmt, ...) {
  ...
  va_end(ap);
  /* reset last_src and last_dst whenever we output some code */
  last_src = -1;
  last_dst = -1;
}

...
static int pflag = 0;
static char rcsid[] = "$Id: x86cygwin.md,v 1.1 2002/08/28 23:12:48 drh Exp $";

#define hasargs(p) (p->syms[0] && p->syms[0]->u.c.v.i > 0 ? 0 : LBURG_MAX)
/*
generated at Tue Aug 18 14:54:21 2020
...
static void progbeg(int argc, char *argv[]) {
        int i;
        extern Interface x86IR, x86cygwinIR;

#define xx(f) assert(!x86cygwinIR.f); x86cygwinIR.f = x86IR.f
        xx(address);
        xx(local);
        xx(x.blkfetch);
        xx(x.blkstore);

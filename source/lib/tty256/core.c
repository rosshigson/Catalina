...
#define byte_tx_buffer(offs)  (((char *)(rxbase+9))+BUFFSIZE+offs)
...
int tty_rxcheck() {
   ...
   //   rx_tail := (rx_tail + 1) & BUFFMASK

   ACQUIRE (lock);
   if (*long_rx_tail != *long_rx_head) {
      rxbyte = *byte_rx_buffer(*long_rx_tail);
      *long_rx_tail = (*long_rx_tail + 1) & BUFFMASK;
   }
   else {
      rxbyte = -1;
   }
...
int tty_tx(char txbyte) {
   ...
   // repeat until (tx_tail <> (tx_head + 1) & BUFFMASK)
   // tx_buffer[tx_head]] := txbyte
   // tx_head := (tx_head + 1) & BUFFMASK

   // if long[@rxtx_mode] & NOECHO
   //   rx()
     
   ACQUIRE (lock);
   while (*long_tx_tail == ((*long_tx_head + 1) & BUFFMASK)) { }
...
int tty_txcheck() {
   ...
   ACQUIRE (lock);
   txbytes = (*long_tx_head + BUFFSIZE - *long_tx_tail) & BUFFMASK;
   RELEASE (lock);
   return BUFFMASK - txbytes;
}

...
#define TX_BUFF_SIZE 64   // bytes
#define RX_BUFF_SIZE 64   // bytes

#define msleep(millisecs) _waitcnt((millisecs)*(_clockfreq()/1000) + _cnt())

/*
 * default buffer space (used for preconfigured ports)
 */
static char txrxbuff[S8_MAX_PORTS * (RX_BUFF_SIZE + TX_BUFF_SIZE)];
 ...
 */
#define long_pin(p)            (*(uint32_t *)(s8base+16+(16*(p))))
#define long_mode(p)           (*(uint32_t *)(s8base+20+(16*(p))))
#define long_baud(p)           (*(uint32_t *)(s8base+24+(16*(p))))

/*
 * Macros to help access initialized SX control block (p = logical pin):
 */
#define byte_p_ctl(p)          (s8base+(p))
#define long_p_head(p)         ((uint32_t *)(s8base+16+(16*(p))))
#define long_p_tail(p)         ((uint32_t *)(s8base+20+(16*(p))))
#define long_p_start(p)        ((uint32_t *)(s8base+24+(16*(p))))
#define long_p_end(p)          ((uint32_t *)(s8base+28+(16*(p))))

/*
 * Macros to help access tx and rx buffers (using port, not logical pin):
 */
#define long_rx_head(port)     ((uint32_t *)(s8base+16+(32*(port))))
#define long_rx_tail(port)     ((uint32_t *)(s8base+20+(32*(port))))
#define long_rx_start(port)    ((uint32_t *)(s8base+24+(32*(port))))
#define long_rx_end(port)      ((uint32_t *)(s8base+28+(32*(port))))
#define long_tx_head(port)     ((uint32_t *)(s8base+32+(32*(port))))
#define long_tx_tail(port)     ((uint32_t *)(s8base+36+(32*(port))))
#define long_tx_start(port)    ((uint32_t *)(s8base+40+(32*(port))))
 ...
 * in place of the initialiation data. Note that only up to S8_MAX_PORTS 
 * ports will be configured and any pins outside the range 0 .. 63 will not 
 * be configured. 
 */
static void autoinitialize() {
   ...
   uint32_t baud;
   uint32_t mode;
   uint32_t rxbuff = (uint32_t)&txrxbuff;
   uint32_t txbuff = (uint32_t)&txrxbuff + RX_BUFF_SIZE;
   ...
   if (s8base != 0) {
      for (port = 0; port < S8_MAX_PORTS; port++) {
         ...
         if ((pin >= 0) && (pin <= 63)) {
            ...
            p_config(p, (1<<7) | (0<<6) | pin, rxbuff, rxbuff, rxbuff, rxbuff + RX_BUFF_SIZE);
         }
         p++; // logical tx pin
         pin = long_pin(p);
         if ((pin >= 0) && (pin <= 63)) {
            ...
            p_config(p, (1<<7) | (1<<6) | pin, txbuff, txbuff, txbuff, txbuff + TX_BUFF_SIZE);
         }
         // update buffer pointers for next logical pin
         rxbuff += RX_BUFF_SIZE + TX_BUFF_SIZE;
         txbuff += RX_BUFF_SIZE + TX_BUFF_SIZE;
      }
   }
}
...
/*
 * s8_openport - set up the smartpins and buffers and enable a port (both rx and tx pins)
 */

 ...
 */
/* $Id: doscan.c,v 1.13 1996/04/01 09:08:59 ceriel Exp $ */
...
o_collect(register int c, register FILE *stream, char type,
			int width, int *basep, char *inp_buf)
{
 ...
 */
static char *
f_collect(register int c, register FILE *stream, register int width, char *inp_buf)
{
	register char *bufp = inp_buf;
	int digit_seen = 0;
   ...
   char	inp_buf[NUMLEN];
	...
	while (1) {
		...
		case 'G':
			...
			if (!width) return done;
			str = f_collect(ic, stream, width, inp_buf);

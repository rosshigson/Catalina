#ifndef __CATALINA_GRAPHICS_H
#define __CATALINA_GRAPHICS_H
...
//
#define FONT_WORDS 183 // capital letters only
...
// plugin codes (used in requests to virtual double buffer cog):
//
#define VDB_setup     1
#define VDB_clear     2
#define VDB_copy      3
...
// offsets into VDI data block (require x_tiles and y_tiles to be defined!)
//
#define MAP_BYTES     (2 * x_tiles * y_tiles) // size in bytes
#define MAP_PADDED    (4 * ((MAP_BYTES + 3) / 4)) // pad to long (for alignment)
#define VGA_PARAMCOUNT 21

// byte offsets from base address of display data
//
#define VGI_ONSCRN    (VGI_OFFSCRN  - MAP_PADDED)
#define VGI_OFFSCRN   (0) 
#define VGI_MAP       (VGI_OFFSCRN  + MAP_PADDED)
#define VGI_MODE      (VGI_MAP      + MAP_PADDED)
#define VGI_VGA       (VGI_MODE     + 4)
#define VGI_CLRS      (VGI_VGA      + 4 * VGA_PARAMCOUNT)
#define VGI_LINE_1    (VGI_CLRS     + 4 * 16) 
#define VGI_LINE_2    (VGI_LINE_1   + 4 * x_tiles)
#define VGI_LINE_3    (VGI_LINE_2   + 4 * x_tiles)
#define VGI_LINE_4    (VGI_LINE_3   + 4 * x_tiles)
#define VGI_SCRN      (VGI_LINE_4   + 4 * x_tiles)
#define VGI_LINE_REQ  (VGI_SCRN     + 4 * 8)
#define VGI_LINE_BUF  (VGI_LINE_REQ + 4)
...
//
// NOTE: you do not need to provide the double buffer bitmap
// - this function gets it automatically.
//
void g_db_setup(int double_buffer);


// Clear the display (if not double buffering), or
// the double buffer bitmap (if double buffering)
//
void g_clear();
...
// Copy does nothing if not double buffering.
//
//   double_buffer  - true if double buffering
//
...
// (use for flicker-free display). The difference
// between Move and Copy is that Move erases the
// original bitmap.
// Move does nothing if not double buffering.
//
//   double_buffer  - true if double buffering
//
...
// Set pixel color to 8 bit RGB pattern
//

#ifndef CATALINA_PLUGIN__H
#define CATALINA_PLUGIN__H
/*
 ...
 *
 *    Bits 15 .. 12  : Cog id containing plugin (or $F if unused)
 *    Bits 11 ..  8  : Lock id (or $F if unused)
 *    Bits  7 ..  0  : Request Code (or $00 if unused)
 *
 * NOTE: to use the entries, the service should be used as a negative
...
/*
 * macros to simplify access to the cog registry. 
 * In each case, c is the cog id (0 .. 7).
 */
...
#define SERVICE_LOCK(s) ((SERVICE_ENTRY(i)>>8) & 0xF)
#define SERVICE_CODE(s) (SERVICE_ENTRY(i) & 0xFF)

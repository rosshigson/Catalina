...
#define byte_rx_buffer(port,offs)  (((char *)(rxbase+81+port*64))+offs)
#define byte_tx_buffer(port,offs)  (((char *)(rxbase+337+port*64))+offs)
...
int s_rxcheck(unsigned port) {
   ...
   //   rxbyte := byte[@rxchar][port] ^ byte[@rx_buffer][(port<<8)+long[@rx_tail][port]]
   //   long[@rx_tail][port] := (long[@rx_tail][port] + 1) & $FF

   ACQUIRE (lock);
   if (*long_rx_tail(port) != *long_rx_head(port)) {
      rxbyte = *byte_rxchar(port) ^ *byte_rx_buffer(port,*long_rx_tail(port));
      *long_rx_tail(port) = (*long_rx_tail(port) + 1) & 0xFF;
   }
   else {
      rxbyte = -1;
   }
...
int s_rxcount(unsigned port) {
   ...
   ACQUIRE (lock);
   rxbytes = (*long_rx_head(port) + 0x10 - *long_rx_tail(port)) & 0xFF;
   RELEASE (lock);
   return rxbytes;
}
...
int s_tx(unsigned port, char txbyte) {
   ...
   // repeat until (long[@tx_tail][port] <> (long[@tx_head][port] + 1) & $FF)
   // byte[@tx_buffer][(port<<4)+long[@tx_head][port]] := txbyte
   // long[@tx_head][port] := (long[@tx_head][port] + 1) & $FF

   // if long[@rxtx_mode][port] & NOECHO
   //   rx(port)
     
   ACQUIRE (lock);
   while (*long_tx_tail(port) == ((*long_tx_head(port) + 1) & 0xFF)) { }
   
   *(byte_tx_buffer(port,*long_tx_head(port))) = txbyte;
   *long_tx_head(port) = (*long_tx_head(port) + 1) & 0xFF;
...
int s_txcheck(unsigned port) {
   ...
   ACQUIRE (lock);
   txbytes = (*long_tx_head(port) + 0x10 - *long_tx_tail(port)) & 0xFF;
   RELEASE (lock);
   return 0xFF - txbytes;
}

...
int pthread_rwlock_timedwrlock(pthread_rwlock_t *rwlock,
   ...
   struct _pthread_node *my_node = NULL;
   struct timespec now;
   int timed_out = 0;
   ...
   // or writers writing, or we time out
   while (1) {
      ...
      if (timed_out) {
         ...
         // we can proceed if there are no readers
         // reading or writers writing

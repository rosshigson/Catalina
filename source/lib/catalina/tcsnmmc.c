...
static unsigned long nmmtd_array[] = {
...
#define NMM_LUT_LIBRARY_array MY_NMM_LUT_LIBRARY_array

#include "nmmtl.inc"

#define LUT_SIZE  (NMM_LUT_LIBRARY_BLOB_SIZE/4)
#define LUT_ADDR  MY_NMM_LUT_LIBRARY_array

#else

// use a #define to rename this so it is not a duplicate!
#define nmmtd_array MY_nmmtd_array
...
/*
 * _threaded_cogstart_NMM_cog : Start a NMM C program in the specified cog,
 ...
 *
 */
int _threaded_cogstart_NMM_cog(uint32_t PC, uint32_t SP, void *arg, unsigned int cog) {
   struct {
      unsigned long REG;
      unsigned long PC;
      unsigned long SP;

...
static unsigned long cmmtd_array[] = {
...
#define CMM_LUT_LIBRARY_array MY_CMM_LUT_LIBRARY_array

#include "cmmtl.inc"

#define LUT_SIZE  (CMM_LUT_LIBRARY_BLOB_SIZE/4)
#define LUT_ADDR  MY_CMM_LUT_LIBRARY_array

#else

// use a #define to rename this so it is not a duplicate!
#define cmmtd_array MY_cmmtd_array
...
/*
 * _threaded_cogstart_CMM_cog : Start a CMM C program in the specified cog,
 ...
 *
 */
int _threaded_cogstart_CMM_cog(uint32_t PC, uint32_t SP, void *arg, unsigned int cog) {
   struct {
      unsigned long REG;
      unsigned long PC;
      unsigned long SP;

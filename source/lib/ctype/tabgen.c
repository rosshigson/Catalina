/* filename.c
 * This C file doesn't have a real comment here yet.
 *
 * $Source: /cvsroot-fuse/tack/xack/src/tools/tabgen/tabgen.c,v $
 * $State: Exp $
 */
...
#ifndef NORCSID
static char *RcsId = "$Id: tabgen.c,v 1.1 2005/05/24 23:22:30 dtrg Exp $";
#endif
...
char *my_getline(char *s, int n, FILE *fp);
	...
	char *name;
{
	...
	while (my_getline(text, BUFSIZE, fp) != NULL) {
		if (text[0] == FILECOM) {
			option(text);
		}
		else {
...
/* Revision history
 * $Log: tabgen.c,v $
 * Revision 1.1  2005/05/24 23:22:30  dtrg

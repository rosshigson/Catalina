...
#define	ALIGNMENT	8
#define _EM_LSIZE 	4	/* added by RJH */
...
/*	This file essentially describes how the mallink info block
	is implemented.
*/
#define MIN_SPLIT 64 // power of 2 (also used as GRABSIZE)
...
private void check_mallinks(const char *s);
private void calc_checksum(mallink *ml);
private void check_work_empty(const char *s);
private void started_working_on(mallink *ml);
private void stopped_working_on(mallink *ml);
...
private void _malloc_link_free_chunk(mallink *ml);
private void _malloc_unlink_free_chunk(mallink *ml);
private mallink *_malloc_first_present(int class);
private mallink *_malloc_search_free_list(int class, size_t n);
...
private void _malloc_truncate(mallink *ml, size_t size);
private void _malloc_combine_chunks(register mallink *ml1, register mallink *ml2);
private mallink *_malloc_create_chunk(void *p, size_t n);
...
#ifdef STORE
#define	MAX_STORE	32
private void _malloc_do_free(mallink *ml);
private void _malloc_sell_out(void);
privatedata mallink *_malloc_store[MAX_STORE];
#endif /* STORE */
...
{_memory_lock(); check_mallinks("malloc entry");{
	...
	*/
	...
	if (ml == MAL_NULL)	{
		/*	Try and extend */
		register void *p;
#define	GRABSIZE	MIN_SPLIT		/* Power of 2 */
		register size_t req =
...
privatedata mallink *_malloc_free_list[MAX_FLIST+1];
...
private void acquire_malout(void);
private void check_ml_last(const char *s);
private void dump_all_mallinks(void);
private void dump_free_list(int i);
private void dump_mallink(const char *s, mallink *ml);
private print_loop(mallink *ml);
private working_on(mallink *ml);
private size_type checksum(mallink *ml);
static FILE *malout;

private mallink *_malloc_free_list_entry(int i);

#define	for_free_list(i,p) \
	for (p = _malloc_free_list_entry(i); p; p = log_next_of(p))
...
dump_free_list(int i)	{
	mallink *ml = _malloc_free_list_entry(i);

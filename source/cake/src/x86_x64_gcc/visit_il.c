...
union __tag4 {
    ...
    signed short signed_int16;
    unsigned short unsigned_int16;
    signed long signed_int32;
    unsigned long unsigned_int32;
    long signed_int64;
    long unsigned_int64;
    float float32;
    double float64;
...
int struct_entry_list_reserve(struct struct_entry_list * p, int n)
{
    if (n > p->capacity)
    {
        ...
        if ((unsigned int)n > 536870911U)
        {
            return 104;
        }
        pnew = realloc(p->data, n * 8U);
        if (pnew == 0)
        {
            return 12;
        }
...
int rpl_snprintf(char * _s, unsigned int n, char * _format, ...);
...
int get_bool_c89_type_specifier(int target);
                    ...
                    __L2:/*case 0*/ 
                    {
                        if (p_type->struct_or_union_specifier)
                        {
                            ...
                            if (p_complete)
                            {
                                ...
                                rpl_snprintf(unique_id, 100U, "%d", p_complete->unique_id);
                                ...
                                if (p == 0)
                                {
                                    ...
                                    if (p_name != 0)
                                    {
                                        ...
                                        rpl_snprintf(new_name, 100U, "__c%s%d", p_complete->tag_name, p_complete->unique_id);
                                        rpl_snprintf(p_complete->tag_name, 200U, "%s", new_name);
                                        _cake_zmem(&i, 64);
                                        i.number = 1;
                                            ...
                                            while (member_declarator)
                                            {
                                                if (member_declarator->declarator)
                                                {
                                                    if (type_is_struct_or_union(&member_declarator->declarator->type))
                                                    {
                                                        ...
                                                        rpl_snprintf(name2, 100U, "%d", p_complete_member->unique_id);
        ...
        else
        {
            if (member_declaration->specifier_qualifier_list != 0)
            {
                if (member_declaration->specifier_qualifier_list->struct_or_union_specifier)
                {
                    ...
                    t.type_specifier_flags = 32768;
                    _cake_zmem(&name, 100);
                    rpl_snprintf(name, 100U, "__m%d", no_name_index++);
                    ss_fprintf(ss, "    ");
...
void d_visit(struct d_visit_ctx * ctx, struct osstream * oss)
{
    ...
    _cake_zmem(&declarations, 16);
    ctx->print_qualifiers = 0;
    ss_fprintf(oss, "/* Cake %s %s */\n", "0.12.21", target_to_string(ctx->options.target));
    ctx->indentation = 0;
        ...
        else
        {
            if (p_iteration_statement->first_token->type == 9015)
            {
                if (p_iteration_statement->declaration)
                {
                    ...
                    ss_fprintf(oss, "{\n");
                    ctx->indentation++;
                    _cake_zmem(&block_scope_declarators, 16);
...
static void d_visit_selection_statement(struct d_visit_ctx * ctx, struct osstream * oss, struct selection_statement * p_selection_statement)
{
    ...
    if (p_selection_statement->first_token->type == 9034)
    {
        ...
        ctx->indentation++;
        _cake_zmem(&name, 100);
        rpl_snprintf(name, 100U, "__v%d", ctx->cake_local_declarator_number++);
        print_identation(ctx, &ss);
        ...
        if (p_member_declaration->member_declarator_list_opt)
        {
            ...
            while (p_member_declarator)
            {
                if (p_member_declarator->declarator)
                {
                    ...
                    {
                        rpl_snprintf(name, 100, "%s", p_member_declarator->declarator->name_opt->lexeme);
            ...
            if (p_member_declaration->specifier_qualifier_list)
            {
                if (p_member_declaration->specifier_qualifier_list->struct_or_union_specifier)
                {
                    ...
                    if (p_complete)
                    {
                        ...
                        {
                            rpl_snprintf(name, 100, "__m%d.%s", no_name_index++, mname);
                            return p_member_declarator;
                        }
                    }
                }
    ...
    /*switch*/
    {
...

        {
            ...
            __L7:/*case 4*/ 
            {
                ...
                {
                    rpl_snprintf(func_name, 200U, "%s", ctx->p_current_function_opt->name_opt->lexeme);
                    rpl_snprintf(name, 100U, "__cake_func_%s", func_name);
                }
                else
                {
                    rpl_snprintf(func_name, 200U, "unnamed");
                    rpl_snprintf(name, 100U, "__cake_func_%p", ctx->p_current_function_opt);
                }
                ...
                if (is_function)
                {
                    ...
                    if (p == 0)
                    {
                        ...
                        if (p_function_defined && (is_static || is_inline || is_auto || is_local_function_definition))
                        {
                            ...
                            _cake_zmem(&local3, 16);
                            _cake_zmem(&local4, 16);
            ...
            __L49:/*case 10*/ 
            {
                ...
                _cake_zmem(&function_literal_nameless, 16);
                ...
                ctx->indentation = 0;
                _cake_zmem(&function_literal_body, 16);
                ...
                _cake_zmem(&function_literal, 16);
                ...
                if (l != 0)
                {
                    rpl_snprintf(generated_function_literal_name, 100U, "__c%zu_f", l->data.number);
                }
                else
                {
                    ...
                    hash_item_set_destroy(&i);
                    rpl_snprintf(generated_function_literal_name, 100U, "__c%d_f", current_cake_declarator_number);
                    _cake_zmem(&lambda_sig, 16);
            ...
            else
            {
                ...
                if (rename_declarator)
                {
                    if (!p_init_declarator->p_declarator->declarator_renamed)
                    {
                        ...
                        _cake_zmem(&name, 100);
                        rpl_snprintf(name, 100U, "__c%d_%s", ctx->cake_file_scope_declarator_number++, p_init_declarator->p_declarator->name_opt->lexeme);

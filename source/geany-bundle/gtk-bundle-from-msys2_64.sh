...
#
#   gtk-bundle-from-msys2_32.sh for 32 bit Geany builds
#   gtk-bundle-from-msys2_64.sh for 64 bit Geany builds
#
# Copy the appropriate version to gtk-bundle-from-msys2.sh 
...
if [[ "$ABI" != "$MSYSTEM_CARCH" ]] ; then
   echo ""
   echo "   ERROR: This script must be executed in an MSYS2 MINGW64 bash shell."
   echo ""
   exit
fi
...
extract_packages() {
	for i in $pkgs; do
		pkg=$(_getpkg $i)
    # echo "PACKAGE $pkg"
    pkg_tail=${pkg:${#pkg}-4:4}
    # echo "PACKAGE TAIL $pkg_tail"
    if [ "$pkg_tail" = ".sig" ]; then
      pkg=${pkg:0:${#pkg}-4}
    fi
    echo "PACKAGE $pkg"

#ifndef __CATALINA_THREADS__H
#define __CATALINA_THREADS__H
 ...
 *             set to 7 by each multithreading kernel on startup - if two 
 *             multithreading kernels are started, they should have their 
 *             locks set to be the same lock, and any lock previously 
 *             allocated should be released by calling _lockclr() on that 
 *             lock - note that this does not apply to the initial lock (7)
 *             as it is not actually allocated by the kernel - it is just used.
 */

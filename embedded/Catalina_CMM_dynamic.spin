...
' kernel_init : initialize VM - note that this code is kept 26 longs long
'               to make various offsets match the main kernel, where this
'               code may be overlaid by debug code once initialization is
'               complete - eventually, the same may be true of this kernel 
'               as well.
'
kernel_init                     '
         mov     r0,par         '$53 1 load my dynamic data block addr
         rdword  reg,r0         '$54 2 load registry address (used later)
         cogid   r2             '$55 3 register ...
         shl     r2,#2          '$56 4 ... ourselves ...
         add     r2,reg         '$57 5 ... as ...
         rdlong  r1,r2          '$58 6 ... a ...
         andn    r1,top8        '$59 7 ... new ...
         wrlong  r1,r2          '$5a 8  kernel
         add     r0,#4          '$5b 9 point to initial PC
         rdlong  PC,r0          '$5c 10 load initial PC
         add     r0,#4          '$5d 11 point to initial SP
         rdlong  SP,r0          '$5e 12 load initial SP
         add     r0,#4          '$5f 13 point to initial arg
         rdlong  r2,r0          '$60 14 load initial arg
         jmp     #read_next     '$61 15 we can now start executing CMM code
         nop                    '$62 16
         nop                    '$63 17
         nop                    '$64 18
         nop                    '$65 19
...
I32_LODF
         ...
         jmp    #done_32
'
reg      long   0               ' use this to hold the registry address

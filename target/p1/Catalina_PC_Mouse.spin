...
CON
  ...
  m_count   = 20                ' 20 contiguous longs
...
PUB start(m_block, pin, baudrate) : okay | cog
...
''
  ...
  long[m_block][rx_pin]    := pin
  long[m_block][rx_mode]   := RXINVERSE
  long[m_block][bit_ticks] := clkfreq / baudrate
  long[m_block][present]   := 1 ' always present
  long[m_block][bufptr]    := m_block + rx_buffer * 4
  okay := cog := cognew(@entry, m_block) + 1

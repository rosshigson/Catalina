...
'' ---  ---------  -----------------------------------------------------
'' 010   7 Jul 08  First version, based on Parallax Graphics Driver v1.0
''                 Supports 2 or 4 colour mode. For use with Virtual VGA Driver.
...
CON
{
  #1, _setup, _color, _width, _plot, _line, _arc, _vec, _vecarc, _pix, _pixarc, _text, _textarc, _textmode, _flush, _loop
}
...
PUB start : cog
  ...
  if (cog => 0)
    ...
    common.Register(cog, common#LMM_VGI)
    ' start the double buffer support driver
    db.start
...
''
''   mode           - 0 means 2 colour mode, <> 0 means 4 colour mode
''   x_tiles        - number of x tiles (tiles are 16x16 pixels in 4 color mode or 32x16 pixels in 2 colour mode)
...
' RJH - 2 or 4 color mode --------------------------------------------------- NEW
  ...
  'of calculations we pretend we have one and it starts at zero. The map and bases are
  'used to translate these virtual addresses to physical tile addresses
  'Note: 50 bases is sufficient for 1600 (2 colour), or 800 (4 color) horizontal pixel resolution.
  repeat n from 0 to x_tiles - 1 <# 50                  'write bases
    bases[n] := n * y_tiles << 6
...
PUB addram(rstart, rend) | n, i 
  ...
  tile_base := (rstart + $003F) & $FFC0                 'align to 64 byte boundary
  ...
  if n > 0  
    ...
    repeat i from 0 to n - 1                            'put additional tiles ...
      ...
      tile_list := tile_base + (i * 64)                 '... of free tiles
...
''   c              - color code in bits[1..0]
  ...
  setcommand(_color, @colors[c & 3])                    'set color (in 2 color mode, colours 2 and 3 are the same as 1)
...
''   w              - 0..15 for round pixels, 16..31 for square pixels
  ...
  p := (w ^ $F) & $F                                    'update slices to new shape/width
  repeat i from 0 to w >> 1
' RJH - 2 or 4 color mode ----------------------------------------------------------------- OLD
'    slices[i] := true >> (p << 1) << (p & $E)          '4 colour mode
' RJH - 2 or 4 color mode ------------------------------------------------------------------NEW
    if colour_mode <> 0
      slices[i] := true >> (p << 1) << (p & $E)         '4 colour mode
    else
      slices[i] := true >> (16+p) << ((16+p)>>1)        '2 colour mode
' RJH ------------------------------------------------------------------------------------- END
...
'' Wait for any current graphics command to flush
'' use this to insure that it is safe to manually manipulate the bitmap

  setcommand(_loop, 0)                                'make sure last command flushed
...
font    word    fline + xa2 + yaC + xb2 + yb7 + more            '!
        ...
        word    farc + xa2 + yaA + aF + ax2 + ay2 + more
        word    farc + xa2 + ya6 + aD + ax2 + ay2 + more
        word    fline + xa0 + ya4 + xb0 + yb6 + more
...
loop                    rdlong  t1,rqst         wz      'wait for command
        if_z            jmp     #loop

                        movd    :arg,#arg0              'get 11 arguments
                        mov     t2,t1
                        mov     t3,#11
:arg                    rdlong  arg0,t2
                        add     :arg,d0
                        ...
                        wrlong  zero,rqst               'zero command to signify received
...
'
'
' setup(x_tiles, y_tiles, x_origin, y_origin, tpm_ptr, map_ptr, n_tiles, tile_ptr)  | t0, t1, tlist_ptr, bases_ptr, slices_ptr, n
'
...
'                        mov     prevaddr,addr           'no - save virtual addr for next time
                        mov     tmp,addr                'convert virtual address ...
                        shr     tmp,#6                  ' ... to tile number (recall that virtual bitmap starts at 0)
        ...
        if_z            jmp     #:have_tile_1           'yes - already have the real tile address
                        ...
                        mov     tpmaddr,tmp             'get ...
                        shl     tpmaddr,#1              '... real tile ...
                        add     tpmaddr,mapptr
                        rdword  tpmaddr,tpmaddr
                        rdword  tileaddr,tpmaddr        ' ... tpm ...
                        mov     tilecolor,tileaddr      ' ... separating ...
                        and     tileaddr,tmask          ' ... tile ...
                        andn    tilecolor,tmask         ' ... from color
                        shl     tileaddr,#6             'tileaddr is the real address of the tile
        ...
        if_nz           jmp     #:have_tile_2           'no - we already have a tile to write on
                        ...
                        tjz     tileaddr,#:use_tile_0   ' if no tiles left, use special 'all 0' tile
...
' RJH ------------------------------------------------------------------------------------- END
        ...
        if_nc           jmp     #plotp_ret
                        ...
                        shl     t1,#2                   'get address of pixel long
                        add     t1,basesptr
                        mov     t2,py
                        rdlong  t1,t1
                        shl     t2,#2
...
' RJH - 2 or 4 color mode ----------------------------------------------------------------- NEW
                        test    cmode,#1        wc
        if_nc           sub     t1,#16                  '2 colour mode
        if_nc           sar     t1,#5
        if_c            sub     t1,#8                   '4 colour mode
        if_c            sar     t1,#4
' RJH ------------------------------------------------------------------------------------- END
                        cmp     t1,xlongs       wc
...
' RJH ------------------------------------------------------------------------------------- END
        if_c            mov     addr,base0
        if_c            movs    mask,#mask0
        if_c            movs    bits,#bits0
...
' RJH - virtual update -------------------------------------------------------------------- NEW
        if_c            mov     addr,base1
        if_c            movs    mask,#mask1
        if_c            movs    bits,#bits1
        if_c            call    #rdwrsparse
' RJH ------------------------------------------------------------------------------------- END
...
cmode                   long    0       'color mode
xlongs                  long    0       'bitmap metrics
ylongs                  long    0

...
v5.2 (20-OCT-12) MODIFIED FOR USE WITH CATALINA BY ROSS HIGSON:
  - remove unused Spin prodedures and VAR parameters (address of parameter
    block is allocated at run time by Setup method).

v5.2 (7.21.06)
- Fixed: The 11KHz version played FM sounds at half the correct frequency
- Fixed: The 11KHz version locked up when channel 8 was used
- If the main loop runs too slow, driver will now slow down instead of hang
...
VAR
  ...
  long pending_shape_ch6
  long pending_freq_ch6
  long pending_duration_ch6
  long pending_volume_ch6
  long pending_amp_env_ch6
  long pending_pcm_start_ch6
  long pending_pcm_end_ch6
}
CON

SND_BLOCK_SIZE = 50*4 ' bytes
...
PUB Setup
'' Setup the sound driver - allocates a buffer, set up sound pin

  ' Allocate a block of Hub memory for buffers
  SND_BLOCK := long[Common#FREE_MEM] - SND_BLOCK_SIZE
  long[Common#FREE_MEM] := SND_BLOCK
  bytefill(SND_BLOCK, 0, SND_BLOCK_SIZE)

PUB Configure(pin)
'' Set up pin to use for sound driver
...
'' returns false if no cog available
  ...
  cog := cognew(@entry, SND_BLOCK)
  if cog => 0 
    Common.Register(cog, Common#LMM_SND)
    ...
    Common.SendInitializationData(cog, SND_BLOCK, 0)
...
channel_data
                        ...
                        long    0,0,0,0,0,0,0,0,21,0,0,0  'Channel 6
end_of_channel_data

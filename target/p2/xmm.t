...
lmm_init
        ...
        nop                     '$6e 21                    
#endif 
#if defined(FLOAT_PINS) && defined(libmc)
' use these spare instructions to set the pin modes, then call fp_setpins
' to set the pin directions (we don't have enough space to do that here)
        wrpin   #FLOAT_PIN_MODE,#FLOAT_CMD_PIN  ' $6f 22
        wrpin   #FLOAT_PIN_MODE,#FLOAT_RSP_PIN  ' $70 23
        wrpin   #FLOAT_PIN_MODE,#FLOAT_AVAL_PIN ' $72 24
        wrpin   #FLOAT_PIN_MODE,#FLOAT_AVAL_PIN ' $72 25
        call    #fp_setpins ' $73 26 set pin directions
#else
        nop                     '$6f 22                    
        nop                     '$70 23                    
        nop                     '$71 24                    
        ...
        nop                     '$73 26       
#endif 
        jmp     #setup_xfer     '$74 setup xfer block and execute C code
'
...
fp_setpins
' could move this to the LUT library if we need more space
        dirh    #FLOAT_CMD_PIN   ' command pin is output
        dirh    #FLOAT_AVAL_PIN  ' A value pin is output
 _ret_  dirh    #FLOAT_BVAL_PIN  ' B value pin is output
...
'------------------------------------------------------------------------------
' fp_operation - request a floating point operation (NOT service!)
' input:   t1          internal fp operation code (1 .. 6, 9)
...
fp_operation
        wxpin   r0,#FLOAT_AVAL_PIN
        wxpin   r1,#FLOAT_BVAL_PIN
        wxpin   t1,#FLOAT_CMD_PIN
.fp_wait
        testp   #FLOAT_RSP_PIN wc                '
  if_nc jmp     #.fp_wait

{{
'------------------------------------------------------------------------------
' Catalina_CMM_Threaded - Implements a multi-threaded Compact Memory Model 
...
'            special values (clock freq etc). Then initialize threading. 
'
Relocate
...
reloc_loop
r13      tjz     BC,#reloc_done '$0d ' no more to copy
...
reloc_done
r20      mov     t1,#0          '$14 ' zero ...
r21      wrlong  t1,req         '$15 ' ... our request block
r22      mov     reg,par        '$16 ' save registry address  
Relocate_ret
r23      ret                    '$17
'
PC       long    0              '$18
...
wait
   ...
   if_z  jmp     #wait          '$58 6 .. (required to relocate segments)
         ...
         sub     SP,#8          '$62 16 reserve space ...
         mov     xfer,SP        '#63 17 .. for xfer block at top of stack
         sub     SP,#4          '$64 18 top of stack will be first thread block
         cogid   t1             '$65 19 set up ...
         shl     t1,#8          '$66 20 ... affinity, flags ...
         wrlong  t1,SP          '$67 21 ... and set ticks to zero
         sub     SP,#33*4       '$68 22 point to begining of thread block
         wrlong  SP,SP          '$69 23 make thread block point to itself
         mov     TP,SP          '$6a 24 make thread block the current thread
         jmp     #read_next     '$6b 25 start executing CMM code
pasm_16 
...
'
read_next
         tjz    SWITCH,#no_swap ' if no context switch code, don't context switch
         djnz   ticks,#no_swap  ' if ticks not yet zero, don't context switch
         mov    t6,PC           ' save PC in t6 (t6 must not be used by context switch code)
         sub    t6,#4           ' correct return address (we return to FC_INLINE or done_32)
         mov    PC,SWITCH       ' point to context switch code
no_swap
...
'
' NOTE IF THESE CHANGE, CHANGE compact_threads_misc.e and catalina_threads.h !!!
'
        fit     $13c
        org     $13c
'
SWITCH  long    0               ' Inject address of CMM_context_switch here
ticks   long    100             ' time till next thread swap (~10 milliseconds)
flags   long    0               ' used for storing flags
lock    long    7               ' lock to use for context switching (default 7)
TP      long    0               ' current thread pointer
reg     long    0               ' registry block address
...
:inst
         ...
         djnz   t1,#:loop       ' continue until all inst loaded
...
'          onto the stack, decrementing the SP. Note that the source
'          and destination data MUST be long aligned - but this is true
'          for all normal structures.
'
I32_PSHB
         add    t2,#3           ' round up the count ...
         andn   t2,#3           ' ... to be a multiple of 4
         sub    SP,t2           ' decrement SP by rounded up size
         mov    t1,r0           ' source is in r0
         mov    t4,SP           ' destination is SP
         jmp    #copy_longs     ' do the copy
'
...
copy_longs
         ...
         rdlong t3,t1           ' read from src to t3
         wrlong t3,t4           ' write t3 to dst
         add    t1,#4           ' increment source
         add    t4,#4           ' increment destination
         sub    t2,#4           ' decrement count ...
         jmp    #copy_longs     ' ... and keep copying
'
I32_MOV     

...
C_P_layS_oundP_C_M__3
 ...
 alignl ' align long
 long I32_BR_B + (@C_P_layS_oundP_C_M__8)<<S32 ' LTI4 reg coni
 alignl ' align long
 long I32_LODI + (@C__sound_lock)<<S32
...
C_P_layS_oundP_C_M__8
 ...
 word I16A_CMPI + (r22)<<D16A + (0)<<S16A
 alignl ' align long
 long I32_BRNZ + (@C_P_layS_oundP_C_M__7)<<S32 ' NEU4 reg coni
 word I16A_MOVI + (r22)<<D16A + (7)<<S16A ' reg <- coni
 word I16A_MOV + (r0)<<D16A + (r22)<<S16A ' setup r0/r1 (2)
 word I16A_MOV + (r1)<<D16A + (r23)<<S16A ' setup r0/r1 (2)
 word I16B_MULT ' MULT(I/U)
 ...
 word I16A_CMPSI + (r22)<<D16A + (0)<<S16A
 alignl ' align long
 long I32_BR_B + (@C_P_layS_oundP_C_M__10)<<S32 ' LTI4 reg coni
 alignl ' align long
 long I32_LODI + (@C__sound_lock)<<S32
 word I16A_MOV + (r2)<<D16A + RI<<S16A ' reg ARG INDIR ADDRG
 word I16A_MOVI + BC<<D16A + 4<<S16A ' arg size, rpsize = 4, spsize = 4

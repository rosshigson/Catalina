...
#include "Catalina_CMM_threaded.spin2"
#else
...
' <<<INSERT INITIAL PASM HERE (if any - must fit in LUT RAM) >>>
 ...
 sub     SP, #8             ' set up ...
 mov     xfer, SP           ' ... xfer block for floating point
 jmp     #\read_next        ' start executing CMM C code
...
#include "Catalina_CMM_kernel_library.inc"
...
'
' C_arg_setup : This fnction must exist because it is invoked by the CMM code 
'               generator, but on the P2 arguments are set up by the code in
'               "Catalina_arguments.inc". This function will be inlined (i.e. 

' Catalina Code

DAT ' code segment
'
' LCC 4.2 for Parallax Propeller
' (Catalina v3.15 Code Generator by Ross Higson)
'

' Catalina Export spi_bootSDCard

 alignl ' align long
C_spi_bootS_D_C_ard ' <symbol:spi_bootSDCard>
 PRIMITIVE(#PSHM)
 long $faa000 ' save registers
 mov r23, r3 ' reg var <- reg arg
 mov r21, r2 ' reg var <- reg arg
 PRIMITIVE(#LODI)
 long @C__spi_buffer
 mov r22, RI ' reg <- INDIRP4 addrg
 cmp r22,  #0 wz
 PRIMITIVE(#BRNZ)
 long @C_spi_bootS_D_C_ard_3 ' NEU4
 mov BC, #0 ' arg size, rpsize = 0, spsize = 0
 PRIMITIVE(#CALA)
 long @C__initialize_spi ' CALL addrg
C_spi_bootS_D_C_ard_3
 cmps r21,  #16 wcz
 PRIMITIVE(#BRAE)
 long @C_spi_bootS_D_C_ard_5 ' GEI4
 PRIMITIVE(#LODL)
 long -1
 mov r0, RI ' reg <- con
 PRIMITIVE(#JMPA)
 long @C_spi_bootS_D_C_ard_2 ' JUMPV addrg
C_spi_bootS_D_C_ard_5
 PRIMITIVE(#LODI)
 long @C__spi_lock
 mov r22, RI ' reg <- INDIRI4 addrg
 cmps r22,  #0 wcz
 PRIMITIVE(#BR_B)
 long @C_spi_bootS_D_C_ard_10 ' LTI4
 PRIMITIVE(#LODI)
 long @C__spi_lock
 mov r2, RI ' reg ARG INDIR ADDRG
 mov BC, #4 ' arg size, rpsize = 4, spsize = 4
 PRIMITIVE(#CALA)
 long @C__acquire_lock ' CALL addrg
C_spi_bootS_D_C_ard_9
C_spi_bootS_D_C_ard_10
 mov r2, #0 ' reg ARG coni
 mov BC, #4 ' arg size, rpsize = 4, spsize = 4
 PRIMITIVE(#CALA)
 long @C_spi_getC_ontrol ' CALL addrg
 PRIMITIVE(#LODL)
 long $1f000000
 mov r20, RI ' reg <- con
 mov r22, r0 ' BANDI/U
 and r22, r20 ' BANDI/U (3)
 cmp r22,  #0 wz
 PRIMITIVE(#BRNZ)
 long @C_spi_bootS_D_C_ard_9 ' NEU4
 mov r2, #21 ' reg ARG coni
 mov BC, #4 ' arg size, rpsize = 4, spsize = 4
 PRIMITIVE(#CALA)
 long @C__locate_plugin ' CALL addrg
 mov r15, r0 ' CVI, CVU or LOAD
 mov BC, #0 ' arg size, rpsize = 0, spsize = 0
 PRIMITIVE(#CALA)
 long @C__cogid ' CALL addrg
 mov r17, r0 ' CVI, CVU or LOAD
 mov r19, #0 ' reg <- coni
C_spi_bootS_D_C_ard_12
 cmps r19, r17 wz
 PRIMITIVE(#BR_Z)
 long @C_spi_bootS_D_C_ard_16 ' EQI4
 cmps r19, r15 wz
 PRIMITIVE(#BR_Z)
 long @C_spi_bootS_D_C_ard_16 ' EQI4
 mov r2, r19 ' CVI, CVU or LOAD
 mov BC, #4 ' arg size, rpsize = 4, spsize = 4
 PRIMITIVE(#CALA)
 long @C__cogstop ' CALL addrg
C_spi_bootS_D_C_ard_16
' C_spi_bootS_D_C_ard_13 ' (symbol refcount = 0)
 adds r19, #1 ' ADDI4 coni
 cmps r19,  #8 wcz
 PRIMITIVE(#BR_B)
 long @C_spi_bootS_D_C_ard_12 ' LTI4
 PRIMITIVE(#LODL)
 long $80000000
 mov r2, RI ' reg ARG con
 mov r3, #1 ' reg ARG coni
 mov BC, #8 ' arg size, rpsize = 8, spsize = 8
 sub SP, #4 ' stack space for reg ARGs
 PRIMITIVE(#CALA)
 long @C_spi_setC_ontrol
 add SP, #4 ' CALL addrg
 mov r22, r17
 or r22, #152 ' BORI4 coni
 shl r22, #24 ' LSHI4 coni
 PRIMITIVE(#LODL)
 long $ffffff
 mov r20, RI ' reg <- con
 and r20, r23 ' BANDI/U (2)
 mov r2, r22 ' BORI/U
 or r2, r20 ' BORI/U (3)
 mov r3, #0 ' reg ARG coni
 mov BC, #8 ' arg size, rpsize = 8, spsize = 8
 sub SP, #4 ' stack space for reg ARGs
 PRIMITIVE(#CALA)
 long @C_spi_setC_ontrol
 add SP, #4 ' CALL addrg
 PRIMITIVE(#LODI)
 long @C__spi_lock
 mov r22, RI ' reg <- INDIRI4 addrg
 cmps r22,  #0 wcz
 PRIMITIVE(#BR_B)
 long @C_spi_bootS_D_C_ard_18 ' LTI4
 PRIMITIVE(#LODI)
 long @C__spi_lock
 mov r2, RI ' reg ARG INDIR ADDRG
 mov BC, #4 ' arg size, rpsize = 4, spsize = 4
 PRIMITIVE(#CALA)
 long @C__release_lock ' CALL addrg
C_spi_bootS_D_C_ard_18
 PRIMITIVE(#LODI)
 long @C__spi_lock
 mov r22, RI ' reg <- INDIRI4 addrg
 cmps r22,  #0 wcz
 PRIMITIVE(#BRBE)
 long @C_spi_bootS_D_C_ard_23 ' LEI4
 PRIMITIVE(#LODI)
 long @C__spi_lock
 mov r2, RI ' reg ARG INDIR ADDRG
 mov BC, #4 ' arg size, rpsize = 4, spsize = 4
 PRIMITIVE(#CALA)
 long @C__lockret ' CALL addrg
C_spi_bootS_D_C_ard_22
C_spi_bootS_D_C_ard_23
 mov r2, #0 ' reg ARG coni
 mov BC, #4 ' arg size, rpsize = 4, spsize = 4
 PRIMITIVE(#CALA)
 long @C_spi_getC_ontrol ' CALL addrg
 PRIMITIVE(#LODL)
 long $1f000000
 mov r20, RI ' reg <- con
 mov r22, r0 ' BANDI/U
 and r22, r20 ' BANDI/U (3)
 cmp r22,  #0 wz
 PRIMITIVE(#BRNZ)
 long @C_spi_bootS_D_C_ard_22 ' NEU4
 mov r2, #0 ' reg ARG coni
 mov BC, #4 ' arg size, rpsize = 4, spsize = 4
 PRIMITIVE(#CALA)
 long @C_spi_getC_ontrol ' CALL addrg
 mov r20, #128 ' reg <- coni
 shl r20, #24 ' LSHI4 coni
 mov r22, r0 ' BANDI/U
 and r22, r20 ' BANDI/U (3)
 cmp r22,  #0 wz
 PRIMITIVE(#BR_Z)
 long @C_spi_bootS_D_C_ard_26 ' EQU4
 mov r13, #1 ' reg <- coni
 PRIMITIVE(#JMPA)
 long @C_spi_bootS_D_C_ard_27 ' JUMPV addrg
C_spi_bootS_D_C_ard_26
 mov r13, #0 ' reg <- coni
C_spi_bootS_D_C_ard_27
 mov r0, r13 ' CVI, CVU or LOAD
C_spi_bootS_D_C_ard_2
 PRIMITIVE(#POPM) ' restore registers
 PRIMITIVE(#RETN)


' Catalina Import _initialize_spi

' Catalina Import _spi_lock

' Catalina Import _spi_buffer

' Catalina Import _locate_plugin

' Catalina Import _release_lock

' Catalina Import _acquire_lock

' Catalina Import _lockret

' Catalina Import _cogstop

' Catalina Import _cogid

' Catalina Import spi_setControl

' Catalina Import spi_getControl
' end

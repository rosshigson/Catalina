...
void	BadCond 		ARGS((int K_, const char *T_));
void	Characteristics 	ARGS((void));
void	Heading 		ARGS((void));
void	History 		ARGS((void));
void	Instructions 		ARGS((void));
void	IsYeqX 			ARGS((void));
void	NewD 			ARGS((void));
void	Pause 			ARGS((void));
void	PrintIfNPositive 	ARGS((void));
FLOAT	Random 			ARGS((void));
void	SR3750 			ARGS((void));
void	SR3980 			ARGS((void));
FLOAT	Sign 			ARGS((FLOAT X_));
void	SqXMinX 		ARGS((int ErrKind_));
void	TstCond 		ARGS((int K_, int Valid_, const char *T_));
void	TstPtUf 		ARGS((void));
int	main 			ARGS((void));
void	msglist 		ARGS((const char **s_));
void	notify 			ARGS((const char *s_));
double	pow 			ARGS((double x_, double y_));
...
#ifdef __CATALINA__
// simulate missing read function ...
read(int k, char ch[], int size) {
    int i;
    for (i = 0; i < size; i++) {
        ch[i]=k_wait();
        if ((ch[i]=='\n')||(ch[i]=='\r')) {
...
#if STDC
double
my_pow(double x, double y) /* return x ^ y (exponentiation) */
#else
double
my_pow(x, y) /* return x ^ y (exponentiation) */
double x, y;
#endif
{
#if 0

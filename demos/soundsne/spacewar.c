 ...
 *            Start               START       
 * 
 *          Use UP/DOWN to choose an option, and A to toggle the option on or off.
 *          START exits the option screen, or restarts the game at the GAME OVER screen.
 *
 ...
 *
 *
 * CATALINA COMPILE OPTIONS:
 *
 *   -C HD_VGA        : resolution is 1920 x 1080 
 *   -C COLOR_4       : use 4 colors (black, red, green & white)
 *   -lgraphic2       : required to enable graphics (Propeller 2)
 *   -lsoundsne       : enable SN Emulator sound effects
 */
...
#ifdef __CATALINA_libsoundsne
#define SFX // enable sound effects
#endif

#if defined(__CATALINA_LARGE) || defined(__CATALINA_SMALL) || defined(__CATALINA_COMPACT)
// run the SFX_Manager periodically rather than using a separete cog
#define SFX_COG 0 
#else
// run the SFX_Manager on a separate cog (gives better sound, but 
// there may not be enough cogs when using XMM or COMPACT mode)
#define SFX_COG 1 
#endif

#define STACK_SPACE 2000 // estimates stack requirement in bytes

#if defined(__CATALINA_LARGE)
#include <hmalloc.h>
#define NUM_TILES 2000 // use a fixed number of tiles in LARGE mode
#endif


#if defined(__CATALINA_P2)
#if !defined(__CATALINA_NO_KEYBOARD)
// keyboard not supported on Propeller 2
#define __CATALINA_NO_KEYBOARD 
#endif
#if defined(__CATALINA_SW_GAMEPAD)
// software gamepad not supported on Propeller 2
#error SW_GAMEPAD NOT SUPPORTED ON PROPELLER 2
#endif
#define CNT _cnt()
#define Cos g_cos  // Cos and Sin ...
#define Sin g_sin  // ... are now in the graphic2 library
#endif
...
#define ANG_ROTATE                  1         
#define SHL_ROTATE                  7          // shl by this amount
...
#define NUM_STARS                   40        
#define NUM_DAMAGE_PARTICLES        10           // particles for each item of damage
#define DAMAGE_TTL_INIT             200         // time to live for damage particles
#define TEXT_SCALE                  2           // general text scaling factor

#define PLAYER_1_STATS_X_POS      (-half_screen_width  + 10)
#define PLAYER_1_STATS_Y_POS      (+half_screen_height - 32)

#define PLAYER_0_STATS_X_POS      (+half_screen_width  - 10/2*16 - 8)
#define PLAYER_0_STATS_Y_POS      (+half_screen_height - 32)

#define STATUS_GAUGE_LENGTH        60
...
byte gamepad_string[]     = "WAITING FOR GAMEPAD DETECTION ...";
byte title1_string[]      = "SPACEWAR!";
byte version_string[]     = "9.0 (CATALINA HD VGA)";
byte title3_string[]      = "WRITTEN BY ERIC MOYER";
byte title4_string[]      = "CATALINA VERSIONS BY ROSS HIGSON";
...
#define MAX_SFX 6 // number of known sound effects
...
sfx_data_t sfx_table[MAX_SFX] = {
   ...
   {0, 3, 0, hit   ,  0, 0, 0xF, 0xF, 0, 0, NULL, NULL, NULL, NULL},
...
#define SFX_SND_SHIELD_SUSTAIN   0x0200 // %00000010_00000000
#define SFX_SND_DAMAGE           0x0400 // %00000100_00000000

#if SFX_COG

// sound effects process stack
#define SFX_STACK_SIZE 500
...
#elif defined(__CATALINA_HD_VGA)
   // HD_VGA uses a USB gamepad, which we re-encode using g_nes().
   // Note that we mask out the X, Y, l & r buttons, which the P1 did 
   // not support - we only use Up, Down, Left, Right, Start, Select, 
   // A & B from each gamepad
   nes_bits = ((g_nes(0)&0xFF)<<8)+(g_nes(1)&0xFF);
#endif
...
void Configure_Options(void *pointer) {
   sint i, y;
   ...
   while (!done) {
      ...
      g_text(-58*TEXT_SCALE, -111, title_start_string);

      g_text(-100*TEXT_SCALE, 83, options_string);
      ...
      while (*option_strings[num_options] != 0xff) {
         ...
         // Draw option name
         g_text(-60*TEXT_SCALE, y, option_strings[num_options]);
         // Draw option setting
         if (game_options & current_option_mask) {
            g_text(60*TEXT_SCALE, y, on_string);
         }
         else {
            g_text(60*TEXT_SCALE, y, off_string);
         }
      ...
      y = 46 -(13 * selected_option);
      g_vec(90*TEXT_SCALE, y, 0x0020*TEXT_SCALE, ANG_180, pointer);
      ...
      ||  (menu_flags & MENU_FLAG__DO_REPEAT_CLICK)) {
        if (nes_buttons & NES_UP) {
           ...
           g_vec(90, y, 0x0020, ANG_180, pointer);
...
#if defined(__CATALINA_P2)
      msleep(50); // on the P2, we need to slow things down a bit
#endif      
   }

}

void main_game(void *rocks, void *shields);
...
// POLYGON OBJECTS (THESE HAVE TO BE DECLARED LOCALLY TO SUPPORT LARGE MODE)
   ...
   player0_ship[0] = player0_ship0;
   player0_ship[1] = player0_ship1;
   player0_ship[2] = player0_ship2; 
   player0_ship[3] = player0_ship3;

   player1_ship[0] = player1_ship0;
   player1_ship[1] = player1_ship1;
   player1_ship[2] = player1_ship2; 
   player1_ship[3] = player1_ship3;

   // get colour mode - on the P1 we can have 0 or 1 (for 2 or 4 color),
   // but on the p2 we only have 2 (for 256 color) which can do either
   color_mode = ((g_mode() == 0) ? 1 : 2);
...
#if defined(__CATALINA_P2) && defined(__CATALINA_COLOR_4)
   // set foreground to white, red, green
   g_palette(1,15); // white
   g_palette(2,9);  // red
   //g_palette(3,12); // blue
   g_palette(3,10); // green
#endif
   ...
   g_text(0, 0, gamepad_string);
   g_copy(DOUBLE_BUFFER);
   while (!g_present(0) && !g_present(1)) {
     _waitsec(1);
   }
   g_clear();
   ...
   while (1) {
     if (color_mode == 2) {
        g_textmode(14,24,6,7); 
     }
     else if ((tile_count > 120) && (color_mode == 0)) {
        g_textmode(7,12,6,5);
     }
     else {
        g_textmode(2,8,6,5);
     }
     g_colorwidth(1,color_mode);
     ...
     g_textmode(2,1,6,4);
     g_text(0, -111*TEXT_SCALE, title_start_string);
     //g_colorwidth(3,3)
     //g_box(-half_screen_width+5,-half_screen_height+5,screen_width-10,screen_height-10)
     g_copy(DOUBLE_BUFFER);
     WaitForStartButton();
     Configure_Options(player0_ship[0]);
     main_game(vectors_rock, vectors_shield);
   }
}
...
void main_game(void *rocks, void *shields) {
   ...
   while ((lives[PLAYER_0] != 0) & (lives[PLAYER_1] != 0)) {
     ...
     object_x[OBJECT_PLAYER_0] = (((half_screen_width) - 64) << 16);
     object_y[OBJECT_PLAYER_0] = (((half_screen_height) - 96) << 16);
     object_x[OBJECT_PLAYER_1] = (-(((half_screen_width) - 64) << 16));
     object_y[OBJECT_PLAYER_1] = (-(((half_screen_height) - 96) << 16));
     ...
     while (died == 0) {
       ...
       g_text(PLAYER_1_STATS_X_POS, PLAYER_1_STATS_Y_POS - 32, fixed_pt_string_0);

       //Draw integer watch variable
       Signed_Int_To_String(int_string_0, watchval_int);
      g_text(PLAYER_0_STATS_X_POS, PLAYER_0_STATS_Y_POS - 32, int_string_0);
#endif         
          ...
          for (i = 0; i < NUM_ROCKS; i++) {
             if (rock_state[i] == ROCK_STATE_PRESENT) {
                ...
                g_vec(object_x[i+OBJECT_ROCK_0]>>16, object_y[i+OBJECT_ROCK_0]>>16, 
                      rock_scale[i], rock_angle[i], rocks);
             }
          ...
          if (!(ship_flags[player_index] & SHIP_FLAG_HYPERSPACE)) {
             ...
             &&  (ship_state[player_index] > SHIP_STATE_DIE_DELAY)) {
                ...
                if (ship_state[player_index] == SHIP_STATE_ALIVE) {
                   ...
                   if (ship_flags[player_index] & SHIP_FLAG_SHIELDS_UP) {
                      g_vec(x, y, SHIP_SCALE, shield_angle, shields);
                   }
       ...
       if (lives[PLAYER_0] != 0) {
          for (i = 1; i < lives[PLAYER_0]; i++) {
             g_vec(PLAYER_0_STATS_X_POS + ((i - 1)<<4) + 4, 
                   PLAYER_0_STATS_Y_POS - 16, 0x0028, (0x2000>>2), player0_ship[0]);
          }
       }
       if (lives[PLAYER_1] != 0) {
          for (i = 1; i < lives[PLAYER_1]; i++) {
             g_vec(PLAYER_1_STATS_X_POS + ((i - 1)<<4) + 4, 
                   PLAYER_1_STATS_Y_POS - 16, 0x0028, (0x2000>>2), player1_ship[0]);
          }
       }
...
#if defined(__CATALINA_P2)
      msleep(20); // on the P2, we need to slow things down a bit
#endif      
     }
   ...
   g_textmode(3*TEXT_SCALE,1*TEXT_SCALE,6,5);
   g_colorwidth(1,0);
   g_text(0, 0, game_over_string);
   g_copy(DOUBLE_BUFFER);
   msleep(5000);

   g_textmode(3*TEXT_SCALE,1*TEXT_SCALE,6,5);
   g_text(0, 0, game_over_restart);
...
// the SNE SFX manager - this function must be called at a rate fast enough
// to ensure that sound deadlines in the various individual VGM sound 
// files are not missed - otherwise strange sounds will result!
...
void SFX_Manager() {
   ...
   do {
      ...
      if ((sfx_flags & SFX_SND_SHIP0_SHOT) != 0) {
         ...
         if ((sfx_cnt_ship0_shot > 0) && (sfx_table[1].state == 0)) {
           sfx_cnt_ship0_shot--;
           sfx_table[1].play = 1;
         }
         else {
      ...
      if ((sfx_flags & SFX_SND_SHIP1_SHOT) != 0) {
         //Play ship 0 shot sound
         if ((sfx_cnt_ship1_shot > 0) && (sfx_table[2].state == 0)) {
           sfx_cnt_ship1_shot--;
           sfx_table[2].play = 1;
         }
         else {
      ...
      // SHIELD SOUNDS
      //-------------------
      if ((sfx_flags & (SFX_SND_SHIP0_SHIELD | SFX_SND_SHIP1_SHIELD)) != 0) {
         //play shield sound
         sfx_table[4].play = 2;
      }
      else {
      ...
      // call the SN Emulator SFX Manager
      sne_sfx();

...
/*
 * Simple test program for the 4 Port serial plugin count functions. 
 * This program only tests one port at a time (as defined by PORT, below).
 *
 * NOTE:
 *
 * By default, the 4 port serial plugin enables port 0 on propeller pins 
 ...
 * For example:
 *
 *   catalina test_count.c -p2 -lci -lserial2 -C NO_HMI
 *
 ...
 * For example, to use the VGA HMI option in addition to the 4 port serial 
 * plugin, compile this program with a command such as:
 *
 *   catalina test_count.c -p2 -lci -lserial2 -lthreads -C LORES_VGA
 */
...
void main() {
   ...
   while (1) {
      ...
      // then print them
      s4_str(PORT, str);
      _waitus(1100); // allow some chars to be sent - change to see effect
      count = s4_txcount(PORT);
      s4_str(PORT,"\n\ntx count = ");
      s4_dec(PORT, count);
      s4_strln(PORT, "");

      count = s4_rxcount(PORT);
      s4_str(PORT,"\nrx count = ");
      s4_dec(PORT, count);
      s4_strln(PORT, "");

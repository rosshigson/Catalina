 ...
 *   0x7BAC in the build_all batch script, which is suitable for most         *
 *   platforms when compiling this program as LMM and using the TTY HMI       *
 *   option. However, it will not be suitable if this program is instead      *
 *   compiled with other options, or as an XMM program. However, the program  *
 *   itself check whether the included programs are compiled to run at the    *
 *   appropriate address and print a message if this is not the case.         *
 *   If the message is displayed, simply edit the build_all batch script      *
 *   and replace 0x7BAC with the correct address (which will be displayed     *
 *   by this program when run). Then rebuild the program and run it again     *
 *                                                                            *
 *   This primary program can be compiled as a CMM, LMM, XMM SMALL or XMM     *
 *   LARGE program.                                                           *
 *                                                                            *
 *   See the build_all batch script for more details.                         *
 *                                                                            *
 ******************************************************************************/
...
void main(int argc, char *argv[]) {
   ...
   if (CMM_CODE_ADDRESS != (int)&RESERVED_SPACE) {
       ...
       printf("address %X - edit the build_all script\n",  &RESERVED_SPACE); 
       while (1);
   }
   ...
   printf("correct address, which is %X.\n\n", &RESERVED_SPACE);
   msleep(1000);

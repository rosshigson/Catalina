...
uint32_t DOSFS_GetNext(PVOLINFO volinfo, PDIRINFO dirinfo, PDIRENT dirent)
{
   ...
   if (dirent->name[0] == 0xe5)   // handle deleted file entries
      dirent->name[0] = 0;
   else if (((dirent->attr & ATTR_LONG_NAME) == ATTR_LONG_NAME) && !( dirinfo->flags & DOSFS_DI_BLANKENT)) //NTRF: don't replace longnames with new files
      dirent->name[0] = 0;
   ...
   do {
      ...
      // End of root directory reached
         ...
         di->currententry = 0; // tempclus is not zero but contains fat entry, so next loop will call
                               // DOSFS_GetNext(), which will increment currententry
...
uint32_t DOSFS_OpenFile(PVOLINFO volinfo, uint8_t *path, uint8_t mode, uint8_t *scratch, PFILEINFO fileinfo)
{
   ...
   while (!DOSFS_GetNext(volinfo, &di, &de)) {
      if (!memcmp(de.name, filename, 11)) {
         // You can use this function call to open a directory if the mode is DOSFS_OPENDIR.
         //NTRF: allows to create directories
         if ((de.attr & ATTR_DIRECTORY) && (mode != DOSFS_CREATEDIR) && (mode != DOSFS_OPENDIR))
            return DOSFS_NOTFOUND;

         // You CANNOT use this function call to open a file if the mode is DOSFS_OPENDIR or DOSFS_CREATEDIR.
         if (!(de.attr & ATTR_DIRECTORY) && ((mode == DOSFS_CREATEDIR) || (mode == DOSFS_OPENDIR)))
            return DOSFS_NOTFOUND;
      ...
      if ( mode == DOSFS_CREATEDIR ) {
          ...
          uint32_t endsector = startsector + volinfo->secperclus - 1;
           ...
           for ( ; endsector > startsector; endsector -= 1 )
              DOSFS_WriteSector( volinfo->unit, scratch, endsector, 1 );
           memcpy( &( de.name ), ".          ", 11 );
          de.filesize_0 = 0;
          de.filesize_1 = 0;
          de.filesize_2 = 0;

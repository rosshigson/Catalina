 ...
 *                                                                            *
 *   catalina -C C3 -C TTY-lc -lm srv_c_p1.c                                  *
 *   payload -o1 -i srv_c_p1                                                  *
 *                                                                            *
...
/* 
 * forward declare the hub_clienT (so the server can start it)
 */
void hub_client(shared_data_t *s);

#pragma catapult primary binary(srv_c_p1) mode(XMM) options(-C FLASH -C CACHED_1K)
 ...
 * specifying type C or Lua.  Note that the list must be terminated
 * with a null entry -i.e. {"", NULL, 0}
 */
...
void main() {
   ...
   // dispatch any calls made to the services in my_service_list
   DISPATCH_C(my_service_list);

}

#pragma catapult secondary hub_client(shared_data_t) address(0x61B0) mode(CMM) stack(500)

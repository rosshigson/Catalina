/*
 * over_p2.c : run secondary programs at a fixed Hub address, loading
 *             the secondary programs from overlay files.
 *             
 * NOTE: This file is configured for a Propeller 2 P2_EDGE board  
 *       and address 0x40000 using the catapult pragmas.
 *                   
 */

#pragma catapult common options(-C P2_EDGE -p2 -O5 -C NO_ARGS -lcx -lma)
...
#pragma catapult secondary func_1 mode(COMPACT) address(0x40000) stack(500) overlay(func_1.ovl)
...
#pragma catapult secondary func_2 mode(TINY) address(0x40000) stack(500) overlay(func_2.ovl)
...
#pragma catapult primary mode(NATIVE) binary(over_p2)

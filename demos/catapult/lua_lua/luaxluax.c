 ...
 *   catapult luaxluax.c                                                      *
 *                                                                            *
 * Then copy luaxluax.bin, first.lua and second.lua to a Catalyst SD card.    *
 * This version only executes compiled Lua programs, so the first.lua and     *
 * second.lua programs must be compiled, using Catalyst commands such as:     *
 *                                                                            *
 *    luac -o first.lux first.lua                                             *
 *    luac -o second.lux second.lua                                           *
 *                                                                            *
 * Then execute them with luaxluax, using a command like:                     *
 *                                                                            *
 *    luaxluax first.lux second.lux                                           *
 *                                                                            *
 * Note that if not specified, the file names default to first.lux and        *
 * second.lux, so you can also just enter:                                    *
 *                                                                            *
 *    luaxluax                                                                *
 *                                                                            *
...
#pragma catapult common options(-p2 -C P2_EDGE -C SIMPLE -C CLOCK -lmc -lcx -lluax)
...
#define DEFAULT_FIRST  "first.lux"
#define DEFAULT_SECOND "second.lux"
 ...
 * library.                                                                   *
 *                                                                            *
 * We allocate 200000 bytes stack space - this can be adjusted down if this   *
 * amount is not required in the second Lua program, and/or more is required  *
 * by the first Lua program.                                                  *
 *                                                                            *
 ******************************************************************************/
#pragma catapult secondary second(shared_data_t) mode(CMM) address(0x71F0) stack(200000) options(-W-w -lthreads linit.c -C LUAX)
 ...
 *                                                                            *
 ******************************************************************************/
#pragma catapult primary binary(luaxluax) mode(XMM) options(-W-w linit.c)

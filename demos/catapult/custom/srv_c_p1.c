 ...
 * definitions (see service.h) and dispatcher (see dispatch_c.c).             *
 *                                                                            *
 * This version is for a Propeller 1 C3 but it could be easily modified to    *
 * suit any Propeller 1 or 2 which has sufficient XMM RAM by modifying the    *
 * catapult pragmas appropriately.                                            *
 *                                                                            *
...
#define MY_SVC_7 (SVC_RESERVED+7)
...
int func_7(char *c1, char *c2) {
    return char_service_2(MY_SVC_7, c1, c2);
}
 ...
 *                                                                            *
 ******************************************************************************/
#pragma catapult primary binary(srv_c_p1) mode(XMM) options(-C FLASH -C CACHED_1K dispatch_c.c)
...
int func_7(char *c1, char *c2) { // example CHAR_2_SVC
   char *result;
   t_printf("Hello, World (from C func_7!)\n");
   result = strstr(c1, c2);
   if (result) {
      // note that Lua indexes start at 1, not 0, so we add 1
      // here so that we get the same result from C and Lua
      return (result - c1) + 1;
   }
   else {
     return 0;
...
void main() {
   ...
   _register_plugin(_cogid(), LMM_SVR2);

   //register the services, so clients can find them
   my_register_services(_locknew(), my_service_list);

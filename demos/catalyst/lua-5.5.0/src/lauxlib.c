...
void *luaL_alloc (void *ud, void *ptr, size_t osize, size_t nsize) {
  ...
  if (nsize == 0) {
    ...
    /* Lua seems to assume that all allocated memory is 
     * initialized to zero, which is not guaranteed by C.
     * Until this dependency can be tracked down and removed, 
     * this routine zeroes all newly allocated memory.
     */
    void *result;
    ...
    if (result != NULL) {
       if (ptr == NULL) {
          ...
          /* initialize only the new part of block */
          memset((char *)result + osize, 0, nsize - osize);
       }
    }
    return result;
  }

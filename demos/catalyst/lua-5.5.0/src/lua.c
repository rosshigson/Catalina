...
static char *lua_readline (char *buff, const char *prompt) {
  if (l_readline != NULL)  /* is there a 'readline'? */
    ...
    return catalina_fgets(buff, LUA_MAXINPUT, stdin);  /* read line */
  }
}
...
/*
** Define Catalina-specific libraries
**
*/
...
static int pmain (lua_State *L) {
  ...
  /* open Catalina-specific libraries */
#if defined(__CATALINA_LUA_LINENOISE)
    /* open linenoise library */
    luaL_requiref(L, "linenoise", luaopen_linenoise, 1);
    lua_pop(L, 1);  /* remove result from the stack */
#endif
#if defined(LUA_PROP)
    /* open propeller and hmi libraries */
    luaL_requiref(L, "propeller", luaopen_propeller, 1);
    lua_pop(L, 1);  /* remove result from the stack */
#endif
#if defined(LUA_HMI)
    luaL_requiref(L, "hmi", luaopen_hmi, 1);
    lua_pop(L, 1);  /* remove result from the stack */
#endif
#if defined(LUA_THREADS)
    /* open threads library */
    luaL_requiref(L, "threads", luaopen_threads, 1);
    lua_pop(L, 1);  /* remove result from the stack */
#endif

...
if lua_version and lua_version < 5.4 then
  -- lua default mode is incremental, just set parameters
  collectgarbage("setpause", 150);
  collectgarbage("setstepmul", 500);
elseif lua_version and lua_version < 5.5 then
  -- set incremental mode and parameters
  collectgarbage("incremental", 150, 500);
else
  -- mode and parameter setting changed in Lua 5.5 ...
...
-- print current top of C heap
function PrintMemory()
  if propeller ~= nil then
    telePrint("Heap top = "..tostring(propeller.sbrk(0)))
  else
    telePrint("Heap top not available")
  end
end
...
function DamageControl()
  ...
  if ShipDocked then
    ...
    local YesNo = string.upper(io.read())
    if YesNo == "Y" or YesNo == 'YES' then
...
-- randomize after waiting some time for user input
...
while (not GameOver) do
  ...
  --if there is very low total energy or shield are damaged, the game is over
    ...
    collectgarbage()
    collectgarbage()
    if propeller ~= nil then propeller.sbrk(1) end
    io.write("? ")
    ...
    elseif Command == 'SHE' or Command == 'SHI' then ShieldControl()
    ...
    elseif Command == 'MEM' then PrintMemory()
    elseif Command == 'XXX' then GameOver = true
      ...
      print("  MEM or Memory     (TO SEE THE TOP OF THE C HEAP)")

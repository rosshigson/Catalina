 ...
 *     the name specified in the initial filename/filesize packet. If no
 *     filename is specified, only the filename in the initial packet is
 *     used, not the path, which means the received file will always end 
                  ...
                  if (packets_rxed == 0) {
                     ...
                     if (i < YM_PACKET_HEADER + 4) {
                        ...
                        if (strlen(filename) == 0) {
                           int len;
                           if ((len = strlen(rx_filename)) > 0) {
                              /* remove path from rx_filename - we identify the
                               * path as anything containing ':', '/' OR '\'
                               * since we don't know if the sender is Windows
                               * or Linux!
                               */
                              while ((len > 0) 
                              && (rx_filename[len - 1] != '/')
                              ...
                              if ((*file = fopen(&rx_filename[len], "wb")) == NULL) {
                                 fprintf(stderr, "\nCannot open file '%s' for write\n",
                                        &rx_filename[len]);
                                 exit(0);
                              }
                              else {
                                 strncpy(filename, &rx_filename[len], FYMODEM_FILE_NAME_MAX_LENGTH);
                              }
                           }
                           else {

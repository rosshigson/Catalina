...
static RVALUE getstringvalue(unsigned char *str);
...
static void decode_using(unsigned char *fmt, int *dp, int *len,
   ...
   static unsigned char using_prefix[256];
   static unsigned char using_suffix[256];
   int plen = 0;
   int slen = 0;
   int saw_hash = 0;
   int saw_strspec = 0;
              ...
              unsigned char *suffix)
{
   if (x.type == INTID) {
      if (x.val.ival >= 0) {
         if ((dollar && (before > 2)) || (before > 0)) {
            if (dollar) {
               sprintf((char *)buff, "%s$%*d%s", 
                       prefix, before-1, x.val.ival, suffix);
            }
            else {
               sprintf((char *)buff, "%s%*d%s", 
                       prefix, before, x.val.ival, suffix);
            }
         }
         else {
            sprintf((char *)buff, "%s%d%s", 
                    prefix, x.val.ival, suffix);
         }
      }
      else {
         if ((dollar && (before > 2)) || (before > 0)) {
            if (dollar) {
               sprintf((char *)buff, "%s-$%*d%s", 
                       prefix, before-2, abs(x.val.ival), suffix);
            }
            else {
               sprintf((char *)buff, "%s-%*d%s", 
                       prefix, before-1, abs(x.val.ival), suffix);
            }
         }
         else {
            sprintf((char *)buff, "%s-%d%s", 
                    prefix, abs(x.val.ival), suffix);
         }
      }
   }
   else {
      if (x.val.dval >= 0.0) {
         if ((before > 0) || (after > 0)) {
            if ((dollar && (before > 2)) || (before > 0)) {
               if (dollar) {
                  sprintf((char *)buff, "%s$%*.*f%s", 
                          prefix, before+after, after, x.val.dval, suffix);
               }
               else {                   
                  sprintf((char *)buff, "%s%*.*f%s", 
                          prefix, before+after+1, after, x.val.dval, suffix);
               }
            }
         }
         else {
            sprintf((char *)buff, "%s%g%s", 
                    prefix, x.val.dval, suffix);
         }
      }
      else {
         if ((before > 0) || (after > 0)) {
            if ((dollar && (before > 2)) || (before > 0)) {
               if (dollar) {
                  sprintf((char *)buff, "%s-$%*.*f%s", 
                          prefix, before+after-1, after, fabs(x.val.dval), suffix);
               }
               else {
                  sprintf((char *)buff, "%s-%*.*f%s", 
                          prefix, before+after, after, fabs(x.val.dval), suffix);
               }
            }
         }
         else {
            sprintf((char *)buff, "%s-%g%s", 
                    prefix, fabs(x.val.dval), suffix);
         }
      }
   }
...
static void do_print(void)
{
  ...
  while (!is_eol(token) && !(token == END) && !(token == ELSE)) {
    if (is_string(token)) {
      ...
      x = expr(1);
      print_to_buff(x, buff, prefix, before, dollar, after, suffix);
      no_lf = 0;
      if (fmt) {
         ...
         // not USING - add spaces required by BASIC
         print_char(' ');
         i = 0;
         while (buff[i]) {
            print_char (buff[i++]);
...
/*
 * do_if - the IF statement. There are three (!) common forms:
 *
 * IF <condition> GOTO <line> [ ELSE <statements> ] [ END IF ]
 * IF <condition> THEN <line> [ ELSE <statements> ] [ END IF ]
 * IF <condition> THEN <statements> [ ELSE <statements> ] [ END IF]
 * IF <condition> THEN END [ ELSE <statements> ] [ END IF]
 *
 * END IF is optional - if not specified, IF statements are terminated by EOL.
 * ELSE is optional. An ELSE clause belongs to the last unterminated IF.
 * Either form can be nested, but the whole statement must be on one line. 
 * if a GOTO jump is taken, returns new line no, else returns 0
 */
static int do_if(void)
{
  RVALUE condition;
  ...
  if (condition.val.ival) {
     ...
     if (jump > 0) {  
        ...
        // special case - accept "THEN <line number>"
        jump = findline(strict_integer(expr(1)));
        return jump;
     }
     else {
...
static int call_assembly(int addr, int arg4, int arg3, int arg2, int arg1) {
   ...
   // Propeller 1 or 2 ...
   PASM(" word I32_LODI+@C_assembly_call<<S32 ' get address to call\n" 
   PASM(" alignl\n");
   PASM(" word I16B_CALI                     ' call assembly function\n");
   PASM(" word I32_LODI+@C_assembly_retn<<S32\n" 
   PASM(" word I16B_PASM\n");
   PASM(" wrlong r0, RI                 ' return assembly result\n");
#else
#ifdef __CATALINA_P2
   // Propeller 2 -  must be NATIVE or TINY mode
   PASM(" rdlong RI, ##@C_assembly_addr ' get address to call\n");
   PASM(" PRIMITIVE(#CALI)              ' call assembly function\n");
   PASM(" wrlong r0, ##@C_assembly_retn ' return assembly result\n");
#else
   // Propeller 1 ... could be SMALL or LARGE
   PASM(" jmp #LODL\n");
   ...
   PASM(" rdlong RI, RI\n");
#endif
   PASM(" jmp #CALI              ' call assembly function\n");
   PASM(" jmp #LODL\n");
...
static void do_input(void)
{
  ...
  while (!is_eol(token) && !(token == END) && !(token == ELSE)) {
     ...
     switch (lv.type) {
       case INTID:
         while (fscanf(fpin, "%d", lv.val.ival) != 1) {
           fgetc(fpin);
           if (feof(fpin)) {
             seterror(ERR_EOF);
             return;
         ...
         break;
       case FLTID:
         while (fscanf(fpin, "%f", lv.val.dval) != 1) {
           fgetc(fpin);
           if (feof(fpin)) {
             seterror(ERR_EOF);
             return;
       ...
       case STRID:
         ...
         end = (unsigned char *)strchr((char *)buff, '\n');
         if (!end) {
...
*/    
  ...
  case RND:
    ...
    answer.type = FLTID;
    answer.val.dval = (rand()/(RAND_MAX + 1.0));
 ...
 *        before returning a dimensioned variable
 */
static int gettoken(void)
{
  int result;

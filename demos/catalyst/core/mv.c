/*
 * simple "mv" command implemented in C for Catalyst. 
 *
 * NOTE: This command is really just a cp followed by an rm of the source!
 *                
 *
...
void do_help() {
   t_eol();
   t_strln("usage mv [options] src [src ...] target");
   t_eol();
   t_strln("options:");
...
void move_file(PVOLINFO vi, char *src, char *dst) {
   ...
   // if everything is ok, we now we delete the src file
   if ((result == DFS_OK) || (result == DFS_EOF)) {
      if (DFS_UnlinkFile(vi, (uint8_t *)src, scratch) != DFS_OK) {
         t_str("cannot remove source ");
         t_strln(src);
      }
   }
   else {
      t_str("error occured while copying ");
      t_strln(src);
   }
...
void move_to_target(PVOLINFO vi, char *src, char *target) {
   uint8_t scratch[SECTOR_SIZE];
   char tgt[MAX_PATH + 1];
   ...
   if (srclen == 0) {
      t_strln("cannot move - no source specified");
      return;
   }

   if (tgtlen == 0) {
      t_strln("cannot move - no target specified");
      return;
   }

   if (strcmp(src, tgt) == 0) {
      t_strln("cannot move - source and target identical");
      return;
   }
   ...
   if (strcmp(target, "..") == 0) {
      t_strln("cannot move to ..");
      return;
   }

   if (strcmp(target, ".") == 0) {
      // since we have no concept of a current directory,
      // moving to "." means moving to the root directory
      target[0] = 0;
   }
   ...
   if (DFS_OpenFile(vi, (uint8_t *)tgt, DFS_OPENDIR, scratch, &fi) == DFS_OK) {
      ...
      if (target_is_dir) {
         t_strln("cannot move - target directory not found");
         return;
      }
   }
   ...
   // finally we are ready to move ... we do this a file at a time 
      ...
      if (DFS_GetVolInfo(0, scratch, pstart, &vi) != DFS_OK) {
         ...
         for (i = 0; i < file_count - 1; i++) {
            if (strlen(files[i]) > MAX_PATH) {
               ...
               move_to_target(&vi, files[i], files[file_count - 1]);
            }
         }
      }
   }

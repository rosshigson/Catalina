...
#define CHUNK_SIZE 2048 // size to move in each read/write - reduce if program is too large!
...
void move_to_target(PVOLINFO vi, char *src, char *target) {
   ...
   if (target_is_dir) {
      ...
      if (pathlen > 0) {
         if (strlen(tgt) + strlen(&src[pathlen]) >= MAX_PATH) {
            t_strln("cannot move - target path too long");
            return;
         }
   ...
   // finally we are ready to move ... we do this a chunk at a time 
         ...
         for (i = 1; i < argc; i++) {
            if ((argv[i][0] != '-') && (argv[i][0] != 0) && (i != target)) {
               if (strlen(argv[i]) > MAX_PATH) {
                  ...
                  move_to_target(&vi, argv[i], argv[target]);
               }
            }
         }
      }

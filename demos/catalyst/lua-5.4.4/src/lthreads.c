...
void luathread_recycle_insert( luathread *lp ) {
  ...
  if ( list_count( &recycle_list ) >= recyclemax ) {
    /* destroy state */
    lua_close( luathread_get_state( lp ));
  } else {
    /* insert lua process in recycle list */
...
/*
** Buffer to store the result of 'func_dump'. It must be initialized
...
static int func_dump (lua_State *L) {
  ...
  state.init = 0;
  if (dump(L, writer, &state, strip) != 0)
...
static int threads_wait( lua_State *L ) {
   ...
   if (nargs > 0) { /* wait for condition */
      ...
      malloc_defragment();
      _thread_allow();
      /* perform TWO garbage collections (becuase some memory 
       * takes two garbage collection cycles to recover!)
       */
...
static int threads_create_newproc( lua_State *L ) {
  ...
  if ( lt == LUA_TFUNCTION ) {
    if ( luathread_copyupvalues( L, lp->lstate, top + 1 ) == FALSE ) {

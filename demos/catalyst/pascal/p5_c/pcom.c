...
// SHOULD REALLY FIX PTOC !!!
//#ifdef __CATALINA__
#define SIGNED_CHAR_OFF 0 
//#else
//#define SIGNED_CHAR_OFF 128
//#endif
...
static void prtlnk(ctp p, integer f)
{
  ...
  { switch (ferrnr) {
    ...
    case 155: cwrite("Control variable must not be declared on intermediate"); break;
...
static void options(boolean* test)
{
  ...
  L1:
    ...
    if (chartp[ch + SIGNED_CHAR_OFF] == illegal) 
      { sy = othersy; op = noop;
        error(399); nextch(&test);
      }
    else
    switch (chartp[ch + SIGNED_CHAR_OFF]) {
      case letter:
        { k = 0; ferr = true;
          do {
            if (k < maxids) 
            ...
            nextch(&test);
          } while (!(inset(chartp[ch + SIGNED_CHAR_OFF], setof(special,illegal,chstrquo,chcolon,
      ...
      case number:
        { op = noop; i = 0;
          ...
          } while (!(chartp[ch + SIGNED_CHAR_OFF] != number));
             ...
             (ch == 'e')) 
            {
              ...
              if (ch == '.') 
                { k = k+1; if (k <= digmax)  digit[k-(1)] = ch;
                  ...
                  if (chartp[ch + SIGNED_CHAR_OFF] != number)  error(201);
                  else
                    do { k = k + 1;
                    ...
                    for( k = 1; k <= i; k ++)
                      {
                        if (val.u.ival <= mxint10) 
                          val.u.ival = val.u.ival*10+ordint[digit[k-(1)] + SIGNED_CHAR_OFF];
      ...
      case chlcmt:
       ...
       break;
      case special:
        { sy = ssy[ch + SIGNED_CHAR_OFF]; op = sop[ch + SIGNED_CHAR_OFF];
          nextch(&test);
        }
        break;
                ...
                { lsp = (structure*)malloc(sizeof(structure)); pshstc(lsp);
                  {
                      ...
                      if (string(with->u.s_subrange.rangetype)) 
...
static void loadaddress(unsigned short* cstptrix, csp cstptr[cstoccmax], integer* topnew, integer* topmax)
{
    if (gattr.typtr != nil) 
      {
        switch (gattr.kind) {
          case cst:   if (string(gattr.typtr)) 
                   if (*cstptrix >= cstoccmax)  error(254);
                   else
                     { *cstptrix = *cstptrix + 1;
                       cstptr[*cstptrix-(1)] = gattr.u.cval.u.valp;
...
static void symbols()
{
  ...
  ssy['+' + SIGNED_CHAR_OFF] = addop;   ssy['-' + SIGNED_CHAR_OFF] = addop;    ssy['*' + SIGNED_CHAR_OFF] = mulop;
  ssy['/' + SIGNED_CHAR_OFF] = mulop;   ssy['(' + SIGNED_CHAR_OFF] = lparent;  ssy[')' + SIGNED_CHAR_OFF] = rparent;
  ssy['$' + SIGNED_CHAR_OFF] = othersy; ssy['=' + SIGNED_CHAR_OFF] = relop;    ssy[' ' + SIGNED_CHAR_OFF] = othersy;
  ssy[',' + SIGNED_CHAR_OFF] = comma;   ssy['.' + SIGNED_CHAR_OFF] = period;   ssy['\'' + SIGNED_CHAR_OFF]= othersy;
  ssy['[' + SIGNED_CHAR_OFF] = lbrack;  ssy[']' + SIGNED_CHAR_OFF] = rbrack;   ssy[':' + SIGNED_CHAR_OFF] = colon;
  ssy['^' + SIGNED_CHAR_OFF] = arrow;   ssy['<' + SIGNED_CHAR_OFF] = relop;    ssy['>' + SIGNED_CHAR_OFF] = relop;
  ssy[';' + SIGNED_CHAR_OFF] = semicolon; ssy['@' + SIGNED_CHAR_OFF] = arrow;
}   /*symbols*/ 
...
static void rators()
{
  ...
  for( i = ordminchar; i <= ordmaxchar; i ++) sop[chr(i) + SIGNED_CHAR_OFF] = noop;
  sop['+' + SIGNED_CHAR_OFF] = plus; sop['-' + SIGNED_CHAR_OFF] = minus; sop['*' + SIGNED_CHAR_OFF] = mul; sop['/' + SIGNED_CHAR_OFF] = rdiv;
  sop['=' + SIGNED_CHAR_OFF] = eqop; sop['<' + SIGNED_CHAR_OFF] = ltop;  sop['>' + SIGNED_CHAR_OFF] = gtop;
}   /*rators*/ 
...
static void chartypes()
{
  ...
  for( i = ordminchar; i <= ordmaxchar; i ++) chartp[chr(i) + SIGNED_CHAR_OFF] = illegal;
  chartp['a' + SIGNED_CHAR_OFF] = letter;
  chartp['b' + SIGNED_CHAR_OFF] = letter; chartp['c' + SIGNED_CHAR_OFF] = letter;
  chartp['d' + SIGNED_CHAR_OFF] = letter; chartp['e' + SIGNED_CHAR_OFF] = letter;
  chartp['f' + SIGNED_CHAR_OFF] = letter; chartp['g' + SIGNED_CHAR_OFF] = letter;
  chartp['h' + SIGNED_CHAR_OFF] = letter; chartp['i' + SIGNED_CHAR_OFF] = letter;
  chartp['j' + SIGNED_CHAR_OFF] = letter; chartp['k' + SIGNED_CHAR_OFF] = letter;
  chartp['l' + SIGNED_CHAR_OFF] = letter; chartp['m' + SIGNED_CHAR_OFF] = letter;
  chartp['n' + SIGNED_CHAR_OFF] = letter; chartp['o' + SIGNED_CHAR_OFF] = letter;
  chartp['p' + SIGNED_CHAR_OFF] = letter; chartp['q' + SIGNED_CHAR_OFF] = letter;
  chartp['r' + SIGNED_CHAR_OFF] = letter; chartp['s' + SIGNED_CHAR_OFF] = letter;
  chartp['t' + SIGNED_CHAR_OFF] = letter; chartp['u' + SIGNED_CHAR_OFF] = letter;
  chartp['v' + SIGNED_CHAR_OFF] = letter; chartp['w' + SIGNED_CHAR_OFF] = letter;
  chartp['x' + SIGNED_CHAR_OFF] = letter; chartp['y' + SIGNED_CHAR_OFF] = letter;
  chartp['z' + SIGNED_CHAR_OFF] = letter; 
  chartp['A' + SIGNED_CHAR_OFF] = letter;
  chartp['B' + SIGNED_CHAR_OFF] = letter; chartp['C' + SIGNED_CHAR_OFF] = letter;
  chartp['D' + SIGNED_CHAR_OFF] = letter; chartp['E' + SIGNED_CHAR_OFF] = letter;
  chartp['F' + SIGNED_CHAR_OFF] = letter; chartp['G' + SIGNED_CHAR_OFF] = letter;
  ...
  chartp['X' + SIGNED_CHAR_OFF] = letter; chartp['Y' + SIGNED_CHAR_OFF] = letter;
  chartp['Z' + SIGNED_CHAR_OFF] = letter; 
  chartp['0' + SIGNED_CHAR_OFF] = number;
  chartp['1' + SIGNED_CHAR_OFF] = number; chartp['2' + SIGNED_CHAR_OFF] = number;
  chartp['3' + SIGNED_CHAR_OFF] = number; chartp['4' + SIGNED_CHAR_OFF] = number;
  chartp['5' + SIGNED_CHAR_OFF] = number; chartp['6' + SIGNED_CHAR_OFF] = number;
  chartp['7' + SIGNED_CHAR_OFF] = number; chartp['8' + SIGNED_CHAR_OFF] = number;
  chartp['9' + SIGNED_CHAR_OFF] = number; chartp['+' + SIGNED_CHAR_OFF] = special;
  chartp['-' + SIGNED_CHAR_OFF] = special; chartp['*' + SIGNED_CHAR_OFF] = special;
  chartp['/' + SIGNED_CHAR_OFF] = special; chartp['(' + SIGNED_CHAR_OFF] = chlparen;
  chartp[')' + SIGNED_CHAR_OFF] = special; chartp['$' + SIGNED_CHAR_OFF] = special;
  chartp['=' + SIGNED_CHAR_OFF] = special; chartp[' ' + SIGNED_CHAR_OFF] = chspace;
  chartp[',' + SIGNED_CHAR_OFF] = special; chartp['.' + SIGNED_CHAR_OFF] = chperiod;
  chartp['\'' + SIGNED_CHAR_OFF]= chstrquo; chartp['[' + SIGNED_CHAR_OFF] = special;
  chartp[']' + SIGNED_CHAR_OFF] = special; chartp[':' + SIGNED_CHAR_OFF] = chcolon;
  chartp['^' + SIGNED_CHAR_OFF] = special; chartp[';' + SIGNED_CHAR_OFF] = special;
  chartp['<' + SIGNED_CHAR_OFF] = chlt; chartp['>' + SIGNED_CHAR_OFF] = chgt;
  chartp['{' + SIGNED_CHAR_OFF] = chlcmt; chartp['}' + SIGNED_CHAR_OFF] = special;
  chartp['@' + SIGNED_CHAR_OFF] = special;

  ordint['0' + SIGNED_CHAR_OFF] = 0; ordint['1' + SIGNED_CHAR_OFF] = 1; ordint['2' + SIGNED_CHAR_OFF] = 2;
  ordint['3' + SIGNED_CHAR_OFF] = 3; ordint['4' + SIGNED_CHAR_OFF] = 4; ordint['5' + SIGNED_CHAR_OFF] = 5;
  ordint['6' + SIGNED_CHAR_OFF] = 6; ordint['7' + SIGNED_CHAR_OFF] = 7; ordint['8' + SIGNED_CHAR_OFF] = 8;
  ordint['9' + SIGNED_CHAR_OFF] = 9;
}
...
int main(int argc, char* argv[])
{
  ...
  /* !!! remove this statement for self compile */
  {
	  int error;
	  if (argc >= 2) {
              rewrite(prr, argv[1], 0, &error); /* open output file */
	      if (error) {

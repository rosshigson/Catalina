...
(*-------------------------------------------------------------------------*)
  ...
  procedure writev(s: strvsp; fl: integer);
...
(*-------------------------------------------------------------------------*)
  ...
  procedure insymbol;
    ...
    var i,k,j: integer;
        ...
        lvp: csp; test, ferr: boolean; p, lp: strvsp;
        iscmte: boolean;
  ...
  1:
        ...
        begin lgth := 0; sy := stringconst;  op := noop;
          ...
          if lgth = 1 then val.ival := ord(string[1])
            ...
            begin new(lvp,strg); pshcst(lvp);
              ...
              with lvp^ do
                begin slgth := lgth; p := nil; sval := nil; j := 1;
                  for i := 1 to lgth do begin
                    if j > varsqt then p := nil;
                    if p = nil then begin 
                      getstr(p); p^.next := nil; j := 1; 
                      if sval = nil then sval := p else lp^.next := p; lp := p
                    end;
                    p^.str[j] := string[i]; j := j+1;
                  end;
                  if p <> nil then for j := j to varsqt do p^.str[j] := ' '
                end;
              val.valp := lvp
    ...
    if dodmplex then begin {  lexical dump }
      ...
      case sy of
         ...
         realconst:   write('real const');
         stringconst: write('string const');
  ...
  begin
    while fcp <> nil do begin
      ts := id; lcases(ts);
      if strequvf(fcp^.name, ts) then goto 1
      else if strltnvf(fcp^.name, ts) then fcp := fcp^.rlink
        else fcp := fcp^.llink;
    end;
1:  fcp1 := fcp
  end (*searchsection*) ;
      ...
      if fp <> nil then
        with fp^ do
          begin write(output,' ':4,ctptoint(*ord*)(fp):intsize(*6*),' '); 
                writev(name, 9); write(' ':4,ctptoint(*ord*)(llink):intsize(*6*),
    ...
    begin
        ...
        begin new(lcp,types); ininam(lcp);
          ...
          while lcp1 <> nil do
            begin
              ...
              if strequvv(lcp1^.name, lcp^.name) then
      ...
      if fwptr <> nil then
        begin error(117); writeln(output);
          repeat write(' type-id '); writev(fwptr^.name, prtlln); writeln;
...
begin
  ...
  if ctpcnt <> 0 then 
     writeln('*** Error: Compiler internal error: identifier recycle balance: ',
             ctpcnt:1);
  if tstcnt <> 0 then 
     writeln('*** Error: Compiler internal error: test recycle balance: ', 
             tstcnt:1);
  if lbpcnt <> 0 then 

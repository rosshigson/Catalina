...
xcopy /s /y "%LCCDIR%"\source\catalina\lua-5.4.8\src src >NUL:
if not exist doc mkdir doc
xcopy /s /y "%LCCDIR%"\source\catalina\lua-5.4.8\doc doc >NUL:
if not exist test mkdir test
xcopy /s /y "%LCCDIR%"\source\catalina\lua-5.4.8\test test >NUL:

cd src
make -B -f Makefile.Catalina.mgw clean
rem make -f Makefile.Catalina.mgw echo
make -B -f Makefile.Catalina.mgw
cd ..

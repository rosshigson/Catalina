...
portable_fini(core_portable *p)
{
    p->portable_id = 0;
    fflush(stdin);
    _waitsec(1); // give messages a chance to be sent
}
...
/* Function: core_start_parallel
        ...
        Two implementations are provided, one using pthreads, one using cogs.
*/
...
int id = 0;
unsigned long stacks[STACK_SIZE* 8]; // MUST BE IN HUB RAM!!!

// define a version of iterate that can be called as a 
// cog function that signals when it is done!
void
cog_iterate(void *res) {
  core_results *r = res;
  //printf("iterating %d times on cog %d starting\n", r->iterations, _cogid());
  iterate(r);
  //printf("iterating on cog %d complete\n", _cogid());
  r->port.done = 1;
...
core_start_parallel(core_results *res)
{
    ...
    res->port.res = 0;
    res->port.done = 0;
    res->port.cog = _cogstart_C(&cog_iterate, res, &stacks[STACK_SIZE*(res->port.id)], STACK_SIZE);
    if (res->port.cog < 0) {
       printf("no more cogs - reduce MULTITHREAD from %d\n", MULTITHREAD);
    }
    else {
...
core_stop_parallel(core_results *res)
{
    do {} while (res->port.done == 0); // wait till done!
    //printf("cog %d stopped\n", res->port.cog);
    return res->port.res;
}

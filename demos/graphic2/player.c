...
void draw_player(struct player* p) {
	...
	if (p->lives > 0) {
		...
		draw_line(p->world_vert[0].x, p->world_vert[0].y, p->world_vert[1].x, p->world_vert[1].y, 0xffffffff);
		draw_line(p->world_vert[1].x, p->world_vert[1].y, p->world_vert[2].x, p->world_vert[2].y, 0xffffffff);
		draw_line(p->world_vert[2].x, p->world_vert[2].y, p->world_vert[0].x, p->world_vert[0].y, 0xffffffff);
	}
	...
	for (i = 0; i < P_VERTS; i++) {
		...
		draw_pixel(p->world_vert[i].x, p->world_vert[i].y, 0xff00ffff);	
	}
	*/
	...
	for (i = 0; i < BULLETS; i++) {
		...
		if (p->bullets[i].alive == TRUE) {
			...
			draw_pixel(p->bullets[i].location.x, p->bullets[i].location.y, 0xffffffff);	
		}
	}

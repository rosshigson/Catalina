...
/*
 * _cogstart_XMM_LARGE_cog : Start a LARGE XMM C program in the specified cog.
...
int _cogstart_XMM_LARGE_cog(uint32_t PC, uint32_t CS, uint32_t SP, 
  ...
  return _cogstart_XMM_LARGE_cog_2(PC, CS, SP, NULL, arg, cog);
}

/*
 * _cogstart_XMM_LARGE_cog_2 : same as above but accepts two arguments
...
#ifdef __CATALINA_LARGE
   for (i = 0; i < 512; i++) {
      kernel[i]=xmmld_array[i];
   }
   cog = _coginit((int)&cog_data>>2, (int)kernel>>2, cog);
#else
   cog = _coginit((int)&cog_data>>2, (int)xmmld_array>>2, cog);
#endif

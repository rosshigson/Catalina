...
/*
 * _cogstart_XMM_SMALL : Start a SMALL XMM C program in the current cog.
 ...
 *
 */
int _cogstart_XMM_SMALL(uint32_t PC, uint32_t CS, uint32_t SP, 
                        void *arg) {
   return _cogstart_XMM_SMALL_cog_2(PC, CS, SP, NULL, arg, ANY_COG);
}

/*
 * _cogstart_XMM_SMALL_2 : same as above but accepts TWO arguments
 *    arg1  : argument to C main function (ends up in r3)
 *    arg2  : argument to C main function (ends up in r2)
 */
int _cogstart_XMM_SMALL_2(uint32_t PC, uint32_t CS, uint32_t SP, 
                          void *arg1, void *arg2) {
   return _cogstart_XMM_SMALL_cog_2(PC, CS, SP, arg1, arg2, ANY_COG);
}

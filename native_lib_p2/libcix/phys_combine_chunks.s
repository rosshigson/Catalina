' Catalina Code

DAT ' code segment
'
' LCC 4.2 for Parallax Propeller
' (Catalina v3.15 Code Generator by Ross Higson)
'

' Catalina Export _malloc_combine_chunks

 alignl ' align long
C__malloc_combine_chunks ' <symbol:_malloc_combine_chunks>
 PRIMITIVE(#PSHM)
 long $f40000 ' save registers
 mov r22, ##-8 ' reg <- con
 adds r22, r2 ' ADDI/P (2)
 rdlong r22, r22 ' reg <- INDIRU4 reg
 mov r23, r22 ' ADDI/P
 adds r23, r2 ' ADDI/P (3)
 mov r22, ##-8 ' reg <- con
 adds r22, r3 ' ADDI/P (2)
 mov r20, r23 ' CVI, CVU or LOAD
 mov r18, r3 ' CVI, CVU or LOAD
 sub r20, r18 ' SUBU (1)
 wrlong r20, r22 ' ASGNU4 reg reg
 mov r22, r2 ' CVI, CVU or LOAD
 mov r20, ##@C_ml_last
 rdlong r20, r20 ' reg <- INDIRP4 addrg
 cmp r22, r20 wz
 if_nz jmp #\C__malloc_combine_chunks_7  ' NEU4
 mov r21, #1 ' reg <- coni
 jmp #\@C__malloc_combine_chunks_8 ' JUMPV addrg
C__malloc_combine_chunks_7
 mov r21, #0 ' reg <- coni
C__malloc_combine_chunks_8
 cmps r21,  #0 wz
 if_nz jmp #\C__malloc_combine_chunks_4 ' NEI4
 mov r22, ##-4 ' reg <- con
 adds r22, r23 ' ADDI/P (2)
 rdlong r20, r22 ' reg <- INDIRP4 reg
 and r20, #3 ' BANDU4 coni
 adds r20, r3 ' ADDI/P (1)
 wrlong r20, r22 ' ASGNP4 reg reg
C__malloc_combine_chunks_4
 mov r22, ##@C_ml_last
 rdlong r22, r22 ' reg <- INDIRP4 addrg
 mov r20, r2 ' CVI, CVU or LOAD
 cmp r22, r20 wz
 if_nz jmp #\C__malloc_combine_chunks_9  ' NEU4
 wrlong r3, ##@C_ml_last ' ASGNP4 addrg reg
C__malloc_combine_chunks_9
 mov r0, #0 ' reg <- coni
' C__malloc_combine_chunks_3 ' (symbol refcount = 0)
 PRIMITIVE(#POPM) ' restore registers
 PRIMITIVE(#RETN)


' Catalina Import ml_last
' end

' Catalina Code

DAT ' code segment
'
' LCC 4.2 for Parallax Propeller
' (Catalina v3.15 Code Generator by Ross Higson)
'

' Catalina Export DFS_GetNext

 alignl ' align long
C_D_F_S__G_etN_ext ' <symbol:DFS_GetNext>
 PRIMITIVE(#NEWF)
 sub SP, #4
 PRIMITIVE(#PSHM)
 long $f80000 ' save registers
 mov r23, r4 ' reg var <- reg arg
 mov r21, r3 ' reg var <- reg arg
 mov r19, r2 ' reg var <- reg arg
 mov r22, r21
 adds r22, #5 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 cmp r22,  #16 wcz 
 if_b jmp #\C_D_F_S__G_etN_ext_5 ' LTU4
 mov r22, r21
 adds r22, #5 ' ADDP4 coni
 mov r20, #0 ' reg <- coni
 wrbyte r20, r22 ' ASGNU1 reg reg
 mov r22, r21
 adds r22, #4 ' ADDP4 coni
 rdbyte r20, r22 ' reg <- CVUI4 INDIRU1 reg
 adds r20, #1 ' ADDI4 coni
 wrbyte r20, r22 ' ASGNU1 reg reg
 rdlong r22, r21 ' reg <- INDIRU4 reg
 cmp r22,  #0 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_7  ' NEU4
 mov r22, r21
 adds r22, #4 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 shl r22, #4 ' LSHU4 coni
 mov r20, r23
 adds r20, #32 ' ADDP4 coni
 rdword r20, r20 ' reg <- CVUI4 INDIRU2 reg
 cmp r22, r20 wcz 
 if_b jmp #\C_D_F_S__G_etN_ext_9 ' LTU4
 mov r0, #1 ' reg <- coni
 jmp #\@C_D_F_S__G_etN_ext_4 ' JUMPV addrg
C_D_F_S__G_etN_ext_9
 mov r2, #1 ' reg ARG coni
 mov r22, r23
 adds r22, #44 ' ADDP4 coni
 rdlong r22, r22 ' reg <- INDIRU4 reg
 mov r20, r21
 adds r20, #4 ' ADDP4 coni
 rdbyte r20, r20 ' reg <- CVUI4 INDIRU1 reg
 mov r3, r22 ' ADDU
 add r3, r20 ' ADDU (3)
 mov r22, r21
 adds r22, #8 ' ADDP4 coni
 rdlong r4, r22 ' reg <- INDIRP4 reg
 rdbyte r5, r23 ' reg <- CVUI4 INDIRU1 reg
 mov BC, #16 ' arg size, rpsize = 16, spsize = 16
 sub SP, #12 ' stack space for reg ARGs
 PRIMITIVE(#CALA)
 long @C_D_F_S__R_eadS_ector
 add SP, #12 ' CALL addrg
 cmp r0,  #0 wz
 if_z jmp #\C_D_F_S__G_etN_ext_8 ' EQU4
 mov r0, ##$ffffffff ' RET con
 jmp #\@C_D_F_S__G_etN_ext_4 ' JUMPV addrg
C_D_F_S__G_etN_ext_7
 mov r22, r21
 adds r22, #4 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 mov r20, r23
 adds r20, #20 ' ADDP4 coni
 rdbyte r20, r20 ' reg <- CVUI4 INDIRU1 reg
 cmps r22, r20 wcz
 if_b jmp #\C_D_F_S__G_etN_ext_13 ' LTI4
 mov r22, r21
 adds r22, #4 ' ADDP4 coni
 mov r20, #0 ' reg <- coni
 wrbyte r20, r22 ' ASGNU1 reg reg
 mov r22, r23
 adds r22, #1 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 cmps r22,  #0 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_15 ' NEI4
 mov r0, #7 ' reg <- coni
 jmp #\@C_D_F_S__G_etN_ext_4 ' JUMPV addrg
C_D_F_S__G_etN_ext_15
 rdlong r22, r21 ' reg <- INDIRU4 reg
 mov r20, ##$fff7 ' reg <- con
 cmp r22, r20 wcz 
 if_b jmp #\C_D_F_S__G_etN_ext_20 ' LTU4
 mov r22, #1 ' reg <- coni
 mov r20, r23
 adds r20, #1 ' ADDP4 coni
 rdbyte r20, r20 ' reg <- CVUI4 INDIRU1 reg
 cmps r20, r22 wz
 if_z jmp #\C_D_F_S__G_etN_ext_19 ' EQI4
C_D_F_S__G_etN_ext_20
 rdlong r22, r21 ' reg <- INDIRU4 reg
 mov r20, ##$ffffff7 ' reg <- con
 cmp r22, r20 wcz 
 if_b jmp #\C_D_F_S__G_etN_ext_17 ' LTU4
 mov r22, r23
 adds r22, #1 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 cmps r22,  #2 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_17 ' NEI4
C_D_F_S__G_etN_ext_19
 mov r22, r21
 adds r22, #12 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 and r22, #1 ' BANDI4 coni
 cmps r22,  #0 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_21 ' NEI4
 mov r0, #1 ' reg <- coni
 jmp #\@C_D_F_S__G_etN_ext_4 ' JUMPV addrg
C_D_F_S__G_etN_ext_21
 mov r0, #5 ' reg <- coni
 jmp #\@C_D_F_S__G_etN_ext_4 ' JUMPV addrg
C_D_F_S__G_etN_ext_17
 rdlong r2, r21 ' reg <- INDIRU4 reg
 mov r3, FP
 sub r3, #-(-4) ' reg ARG ADDRLi
 mov r22, r21
 adds r22, #8 ' ADDP4 coni
 rdlong r4, r22 ' reg <- INDIRP4 reg
 mov r5, r23 ' CVI, CVU or LOAD
 mov BC, #16 ' arg size, rpsize = 16, spsize = 16
 sub SP, #12 ' stack space for reg ARGs
 PRIMITIVE(#CALA)
 long @C_D_F_S__G_etF_A_T_
 add SP, #12 ' CALL addrg
 mov r22, r0 ' CVI, CVU or LOAD
 wrlong r22, r21 ' ASGNU4 reg reg
C_D_F_S__G_etN_ext_13
 rdlong r22, r21 ' reg <- INDIRU4 reg
 sub r22, #2 ' SUBU4 coni
 mov r20, r23
 adds r20, #20 ' ADDP4 coni
 rdbyte r20, r20 ' reg <- CVUI4 INDIRU1 reg
 #ifndef NO_INTERRUPTS
  stalli
 #endif
 qmul r22, r20 ' MULU4
 getqx r0
 #ifndef NO_INTERRUPTS
  allowi
 #endif
 mov r2, #1 ' reg ARG coni
 mov r20, r23
 adds r20, #48 ' ADDP4 coni
 rdlong r20, r20 ' reg <- INDIRU4 reg
 mov r22, r20 ' ADDU
 add r22, r0 ' ADDU (3)
 mov r20, r21
 adds r20, #4 ' ADDP4 coni
 rdbyte r20, r20 ' reg <- CVUI4 INDIRU1 reg
 mov r3, r22 ' ADDU
 add r3, r20 ' ADDU (3)
 mov r22, r21
 adds r22, #8 ' ADDP4 coni
 rdlong r4, r22 ' reg <- INDIRP4 reg
 rdbyte r5, r23 ' reg <- CVUI4 INDIRU1 reg
 mov BC, #16 ' arg size, rpsize = 16, spsize = 16
 sub SP, #12 ' stack space for reg ARGs
 PRIMITIVE(#CALA)
 long @C_D_F_S__R_eadS_ector
 add SP, #12 ' CALL addrg
 cmp r0,  #0 wz
 if_z jmp #\C_D_F_S__G_etN_ext_23 ' EQU4
 mov r0, ##$ffffffff ' RET con
 jmp #\@C_D_F_S__G_etN_ext_4 ' JUMPV addrg
C_D_F_S__G_etN_ext_23
C_D_F_S__G_etN_ext_8
C_D_F_S__G_etN_ext_5
 mov r2, #32 ' reg ARG coni
 mov r22, r21
 adds r22, #5 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 shl r22, #5 ' LSHI4 coni
 mov r20, r21
 adds r20, #8 ' ADDP4 coni
 rdlong r20, r20 ' reg <- INDIRP4 reg
 mov r3, r22 ' ADDI/P
 adds r3, r20 ' ADDI/P (3)
 mov r4, r19 ' CVI, CVU or LOAD
 mov BC, #12 ' arg size, rpsize = 12, spsize = 12
 sub SP, #8 ' stack space for reg ARGs
 PRIMITIVE(#CALA)
 long @C_memcpy
 add SP, #8 ' CALL addrg
 rdbyte r22, r19 ' reg <- CVUI4 INDIRU1 reg
 cmps r22,  #0 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_25 ' NEI4
 mov r22, r21
 adds r22, #12 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 and r22, #1 ' BANDI4 coni
 cmps r22,  #0 wz
 if_z jmp #\C_D_F_S__G_etN_ext_27 ' EQI4
 mov r22, r21
 adds r22, #5 ' ADDP4 coni
 rdbyte r20, r22 ' reg <- CVUI4 INDIRU1 reg
 adds r20, #1 ' ADDI4 coni
 wrbyte r20, r22 ' ASGNU1 reg reg
 mov r0, #0 ' reg <- coni
 jmp #\@C_D_F_S__G_etN_ext_4 ' JUMPV addrg
C_D_F_S__G_etN_ext_27
 mov r0, #1 ' reg <- coni
 jmp #\@C_D_F_S__G_etN_ext_4 ' JUMPV addrg
C_D_F_S__G_etN_ext_25
 rdbyte r22, r19 ' reg <- CVUI4 INDIRU1 reg
 cmps r22,  #229 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_29 ' NEI4
 mov r22, #0 ' reg <- coni
 wrbyte r22, r19 ' ASGNU1 reg reg
 jmp #\@C_D_F_S__G_etN_ext_30 ' JUMPV addrg
C_D_F_S__G_etN_ext_29
 mov r22, #15 ' reg <- coni
 mov r20, r19
 adds r20, #11 ' ADDP4 coni
 rdbyte r20, r20 ' reg <- CVUI4 INDIRU1 reg
 and r20, #15 ' BANDI4 coni
 cmps r20, r22 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_31 ' NEI4
 mov r22, r21
 adds r22, #12 ' ADDP4 coni
 rdbyte r22, r22 ' reg <- CVUI4 INDIRU1 reg
 and r22, #1 ' BANDI4 coni
 cmps r22,  #0 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_31 ' NEI4
 mov r22, #0 ' reg <- coni
 wrbyte r22, r19 ' ASGNU1 reg reg
 jmp #\@C_D_F_S__G_etN_ext_32 ' JUMPV addrg
C_D_F_S__G_etN_ext_31
 rdbyte r22, r19 ' reg <- CVUI4 INDIRU1 reg
 cmps r22,  #5 wz
 if_nz jmp #\C_D_F_S__G_etN_ext_33 ' NEI4
 mov r22, #229 ' reg <- coni
 wrbyte r22, r19 ' ASGNU1 reg reg
C_D_F_S__G_etN_ext_33
C_D_F_S__G_etN_ext_32
C_D_F_S__G_etN_ext_30
 mov r22, r21
 adds r22, #5 ' ADDP4 coni
 rdbyte r20, r22 ' reg <- CVUI4 INDIRU1 reg
 adds r20, #1 ' ADDI4 coni
 wrbyte r20, r22 ' ASGNU1 reg reg
 mov r0, #0 ' reg <- coni
C_D_F_S__G_etN_ext_4
 PRIMITIVE(#POPM) ' restore registers
 add SP, #4 ' framesize
 PRIMITIVE(#RETF)


' Catalina Import DFS_GetFAT

' Catalina Import DFS_ReadSector

' Catalina Import memcpy
' end
